...
Arriving and departing frames are carried too, but not by FLIP, which has nothing to say about either: a frame that was not on screen when the settle armed has no First rect to invert, and a frame that left is gone before any effect could run on it. An arriving frame plays its own entrance at the geometry the commit gave it — a fade up over a short rise, `PANE_ENTER_RISE_PX` — on its own TugAnimator slot so it never cancels a neighbour's geometry tween. A departing one leaves a **ghost**: a tile at the frame's last measured rect, faded over `PANE_EXIT_GHOST_MS` and removed when the fade lands, carrying the **face** the pane had — a deep clone taken on `cardWillBeginDestruction`, the one notification that fires while the frame is still mounted — so a card fades as itself rather than as a coloured rectangle. Departures are derived from the First measurement rather than from any close handler — a pane the arm measured that no longer has a frame has left, whichever path closed it — which is what keeps the treatment from being one button's manners.

### The ghost's contract {#ghost-contract}

**A copy of a live card, left standing in the document for a beat, is a thing that can leak back into the running app in exactly three ways — by placing itself, by answering to something, or by never leaving.** It leaked the first way within a day of the face landing, and would have leaked the others in time. So each of the three is closed by one rule, and each rule is a list or a record rather than a fix applied where the last defect was found.

- **The ghost owns geometry; the face owns none.** The ghost is placed at the rect the arm measured. The face fills it through `.tug-pane-exit-ghost > .tug-pane-exit-face` and nothing else, which is true by construction rather than by specificity: every inline geometry property the clone carried off the live frame is stripped from its root (`FACE_GEOMETRY_PROPS`, `departure-face.ts`). The live frame's `style` carries the imposer's `position: absolute` and its `left` calc, `cloneNode` keeps the attribute whole, and inline geometry outranks any class rule — which is why the copy once stood a whole card's width from where the card had been. A ghost carrying a face also stands its own tile chrome down, so the face's border is the only one and `inset: 0` lands the face exactly on the measured rect.
- **The face answers to nothing.** Every attribute by which the deck, the harness or the focus machinery addresses a live node is stripped from the face and from every node under it (`FACE_IDENTITY_ATTRS`). The list is not a guess: it is what a census found actually resolving inside a standing ghost — focus targets, a scroll key, a card host, a list cell. The queries that reach for them are document-wide, so a still is not merely reachable but reachable *first* when it sorts before the live card in document order. **The price is appearance and it is paid on purpose:** a few rules key on those attributes, and a departing transcript's commit chips flatten for the length of the fade rather than the contract acquiring an exception.
- **One record owns every ghost's lifetime.** A ghost is registered the instant it is planted and removed by name at every way out of a settle: the `depart` beat's landing, a retarget's `arm`, the window sweep, and the canvas unmount. The retarget's exit is the precise one — a ghost whose fade is in flight has a landing coming that runs unconditional on the generation and is left to fade, while a ghost whose beat never launched has nothing coming for it at all and would otherwise stand for the life of the canvas. Sweeping both would cut a departure short the moment a second close landed beside it, and two cards closed in one gesture each keep their own ghost and their own face.

`.tug-pane-exit-ghost` and `.tug-pane-exit-face` are the only two selectors that style a ghost. No `.tug-pane` rule keyed on a `data-*` attribute may change its box; a rule that wants to tune the fade is written on the ghost's own selectors. The gates are `at0582` (the rect, read on every animation frame of the `depart` beat against the rect the arm measured) and `at0583` (the selector census and the inertness reading), with `at0450` holding the lifetime — no ghost at rest, including after a close a second arrangement change interrupted.

**Whether the face should be a clone at all is deferred, not settled.** A `cloneNode` of a live card is an open-ended leak class: the rules above close it by enumeration where they can and by census where they cannot, and both lists are only as complete as the last census run. A rasterized still would have no styles to leak and no selectors to answer, at the cost of a paint the deck does not take anywhere else today. **What reopens the question is a third leak of this class** — a property or an attribute the clone carried that the ghost did not expect, found in the running app rather than by the censuses. Two have been found and closed; a third would say enumeration is not converging. The larger alternative — keeping the real frame mounted through a `departing` mark, symmetric with `arriving`, which would make the ghost, the face and both lists unnecessary — is its own change and is also deferred rather than rejected.
