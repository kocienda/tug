...
The S5 settle is the reference implementation of the qualifying form on a structural surface (`deck-canvas.tsx`, `lib/pane-flip.ts`). The old motion transitioned `left`/`top`/`bottom`/`width` — per-frame *layout* of every moving pane's subtree, a cost class above even the walk. Now: React commits the final geometry in one layout pass; each moved frame is tweened from the inverse delta to identity — measured First in a store subscriber (pre-commit), Last in a layout effect (post-commit, pre-paint), spring shape in 33 sampled keyframe offsets under keyword `linear`, run through TugAnimator with `fill: 'none'` and every landing path clearing the inline residue. A frame with zero delta gets no animation at all; reduced motion skips tween, measurement, and hold alike — the snap *is* the settle.
...
Height is the axis that cannot be smeared: the gestures that change a frame's height — a stack, a split, a join or a leave under a split — halve or double it, and no scale cap admits that ([D135]). So height crosses as a real `height` term, main-thread for the frame's own box, and what keeps that from being the old cost class is that the term is paid for the frame and **not for its subtree**. Every frame whose settle carries a height term is marked a **still crossing** (`data-still-crossing`, `lib/fold-crossing.ts`) for the life of its beat chain; under the mark the pane's content box clips and the card's root is held at the larger of its First and Last content heights, so the interior is laid out once and only the frame's edge moves. A subtree with a definite height that does not change is not dirtied by its ancestor's tween: a list view's container `ResizeObserver` — which answers every delivery with a re-pin and a render — delivers nothing mid-tween, and the local commits the notification hold was never going to defer (#residency) are removed at their source rather than deferred. Under FLIP the hold is nearly free: on a growth the interior already stands at the final height when the mark goes on, and takes its one delivery at launch, before the first paint; on a shrink the hold restores the First height pre-paint and the one relayout lands at release. The held picture hangs from the top unless the card's root declares `data-still-anchor="bottom"`, which a session card writes while its transcript follows the bottom, so composer and status row ride the frame's edge; a fold crossing is one kind of still crossing and keeps its own top-anchored picture. The gate is `at0605` (no delivery inside a held session card between the first painted frame and landing, on a stack, a split and a two-card join). The release price of the held form against the unheld one has not been benched; the `height` tween itself stays main-thread, so the per-frame walk remains.
...
Arriving and departing frames are carried too, but not by FLIP, which has nothing to say about either: a frame that was not on screen when the settle armed has no First rect to invert, and a frame that left is gone before any effect could run on it. An arriving frame plays its own entrance at the geometry the commit gave it — a fade up over a short rise, `PANE_ENTER_RISE_PX` — on its own TugAnimator slot so it never cancels a neighbour's geometry tween. A departing one leaves a **ghost**: a blank tile at the frame's last measured rect, carrying the pane's ground, border and radius and nothing else, faded over the `divide-join` recipe's `0.6×` window and removed when the fade lands. Departures are derived from the First measurement rather than from any close handler — a pane the arm measured that no longer has a frame has left, whichever path closed it — which is what keeps the treatment from being one button's manners.

### The ghost's contract {#ghost-contract}

**A departure is a blank tile standing at the frame's last measured rect, and it is held to three rules so that a card leaving cannot leak back into the running app.** The rules were written against a *clone* of the live card that stood there for one day; the clone is retired and the rules are what survived it, because each of them is about the mark a departure leaves rather than about what the mark is made of.

- **The tile owns its geometry, and nothing else places it.** It is planted at the rect the arm measured, `position: fixed`, with its four numbers written inline by the pass that plants it. This is the rule the clone broke first and worst: the live frame's `style` carries the imposer's `position: absolute` and its `left` calc, `cloneNode` keeps the attribute whole, and inline geometry outranks any class rule — so the copy stood **1136px** from where the card had been, on an ordinary slot-2 pane. A mark that is not where the thing was is not a mark of it leaving.
- **The tile answers to nothing, and it carries nothing.** Nothing stands inside a ghost. That is now true by construction rather than by enumeration, and the enumeration is why: every attribute by which the deck, the harness or the focus machinery addresses a live node had to come off a clone and off every node under it, and the list of such attributes is open — a census found focus targets, a scroll key, a card host and a list cell resolving inside a standing ghost after six had already been stripped. The queries that reach for them are document-wide, so a still is not merely reachable but reachable *first* when it sorts before the live card in document order.
- **One record owns every ghost's lifetime.** A ghost is registered the instant it is planted and removed by name at every way out of a settle: the `depart` beat's landing, a retarget's `arm`, the window sweep, and the canvas unmount. The retarget's exit is the precise one — a ghost whose fade is in flight has a landing coming that runs unconditional on the generation and is left to fade, while a ghost whose beat never launched has nothing coming for it at all and would otherwise stand for the life of the canvas. Sweeping both would cut a departure short the moment a second close landed beside it, and two cards closed in one gesture each keep their own ghost.

`.tug-pane-exit-ghost` is the only selector that styles a ghost. No `.tug-pane` rule keyed on a `data-*` attribute may change its box; a rule that wants to tune the fade is written on the ghost's own selector. The gates are `at0582` (the rect and the tile's emptiness, read on every animation frame of the `depart` beat against the rect the arm measured) and `at0583` (the selector census and the pointer-events reading). The lifetime — no ghost at rest, including after a close a second arrangement change interrupted — was held by `at0450` until that census was retired on 2026-09-18, and is unpinned until something replaces it.

**The clone was tried, measured and retired, and this is the record so nobody re-adds it as an obvious improvement.** Nobody asked for it: it arrived inside an arc about a card's opening height, named in one subordinate clause of that arc's commit message, with no brief and no decision behind it. It cost two user-visible defects in two days, and both are the same defect wearing different clothes — a copy of a card is everything about the card that is not DOM, left behind.

- **Scroll offsets are element state, not DOM, so `cloneNode` does not carry them.** On a card scrolled to 1200px the census read **598 of 728 interior nodes displaced, worst 1200.0px** — 82% of the card snapping to its top for the length of the fade. The offset is the one quantity guaranteed non-zero on the card a reader watches most: a transcript is pinned to its bottom.
- **Stripping `data-slot` for the identity rule above reverted real box metrics.** There are 67 CSS rules keyed on `[data-slot]` and they are not all colour. A file-chooser button went from **28px wide to 50px** when the attribute came off, shoving the input beside it from x=119 to x=139 and shrinking it 570→550; the census read **47 of 134 interior nodes moved, worst 22.0px**. The strip list and the appearance it costs were in direct contradiction, and no ordering of the two lists resolves it.

The rest of the class is read from the platform rather than measured, and it has no end: `<canvas>` bitmaps do not survive `cloneNode`, `input.value` is a property rather than an attribute, and CSS animations restart from zero in a copy. Enumerating them was the approach, and it did not converge.

**Keeping the real frame mounted through a `departing` mark — symmetric with `arriving`, inert and excluded from the imposer's arithmetic through the fade — remains the right end state, and it is deferred rather than rejected.** It is the only answer that makes this whole class impossible rather than individually fixed. Its one hard part is excluding a departing pane from the placement solver, which is substantial work, and it is not paid for until the blank tile is judged and found wanting. A rasterized still is third of three: it photographs a scroller faithfully and has no selectors to answer, at the cost of a paint on every close, and it is still a snapshot.

### The accepted snaps {#accepted-snaps}

The settle's promise is that a layout change travels rather than jumps, and `tests/app-test/at0450-imposer-cut-census.test.ts` held it to that by watching every frame for the length of a gesture: it reported any pane that changed places, or arrived, with nothing animating it. The census was retired on 2026-09-18, with the promise broken for exactly one gesture — a drop-zone commit cutting four frames by 164px — so the promise below now stands unpinned. Four movements are deliberately outside that promise. They are recorded here so a reader meets them as decisions rather than finding them as bugs, and they are absent from the census for the same reason.
