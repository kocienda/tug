...
| Find reveal (the live `revealRange` placing its rect) | `TugListViewHandle.revealRange`, which disengages follow-bottom under the caller's source and stays live across mounts and re-measurements | `SmartScroll.isUserScrolling`, or `scrollTop` drifts more than `SCROLL_CORRECTION_SUPERSEDE_DRIFT_PX` (8px) from the reveal's own last write read-back (`armedTop`) | `checkRangeRevealCancel` in `tug-list-view.tsx`, run at the top of every advance **and from the list's `onScroll`** — the native scrollbar raises no `isUserScrolling`, so the drift read-back is the only thing that catches it |
