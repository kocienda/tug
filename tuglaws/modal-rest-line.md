...
**Compacting** stood beside it on that reasoning until the fold became a door the run admits, and now takes the line. The argument that retired the exemption is worth keeping: the cover is not a panel over a card with nothing behind it, it is one *face* of a run whose other face is the folded card's Z2 row. Fold the card and the cover lowers into the row on the imposer's clock; unfold it and the cover rises back out of the row at the crossing's end. The line it rises from is the line the other face stands on, so what the motion reveals is the handoff — which is the one thing a reader of a compacting card needs to be shown.
