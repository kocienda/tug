...
**The atom answers the pointer, and a commit atom's primary act is its card.** A click on a commit shows the commit: its own card, carrying the record the History shade's expanded row carries, under a masthead whose three lines are the pill, the subject, and author · date · time. That holds wherever the pill stands — written into a sentence, placed in a History row, placed in a receipt header — and it is the rule an atom names an object for. The diff the click used to open is one *view* of a commit rather than the commit itself, so it stays on the menu, a row below Open Commit.

The pointer reads that promise. A pill whose host gave it an act takes the pointer cursor; a pill with no act stays what it was, cursor `default` and colour unmoved, because a link's hover would promise a navigation that never comes. The border firms from 30% to 45% either way, which is what every identity pill does under a pointer, authored on the atom rather than on each of its hosts. Gesture ownership is still the host's — `CommitShaText` stops every pointer gesture on the sha so a right-click cannot fold the row out from under its own menu — and what `onActivate` decides is whether the stopped click is anything at all.
...
**The baseline is the label's, by the strut.** The borrowed skin's zero-width `::before` in the pill's own face, centred with the label, is what makes the container offer the line the baseline a reader is actually looking at — the label's, not the box's and not the node's. No `vertical-align` goes with it, on the atom or on any host: a length there is a correction layered over a lie, and it drifts the moment the register or the face moves. One app-test pins the number, in the surface where it is hardest to hold: in a transcript sentence the pill's label baseline and the neighbouring word's differ by no more than a pixel, at the register. [type-alignment.md](type-alignment.md) states the same discipline for text generally, and its rule (iv) is this rule's **sibling rather than its parent**: an inline SVG glyph takes one declared em token (`--tug-glyph-cap-offset`), where the atom takes the baked renderers' per-instance offset — two answers because a baked image's metrics are known at bake time and a vector in a line box's are not.
