...
**A mention runs small; a placed atom takes its host's size.** A confirmed sha in prose is set at `0.85em` of the text around it (`--tugx-atom-commit-size`, defaulted in the `var()` fallback in `tug-annotation.css`): it is a fact the reader can reach for, not a word in the sentence's own voice, and at full size ten characters of hex take more of a line than the fact deserves. An **atom** does the opposite, because an atom is rarely alone on its line — a History row and a `/commit` receipt both lead with `commit:<8ch>` and set the subject immediately beside it, and a sha shrunk there reads as a different, lesser kind of text than the words it introduces. The atom therefore inherits, as every other atom label does, and the small size is the mention's alone.
