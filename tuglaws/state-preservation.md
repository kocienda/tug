...
function SessionCardContent() {
  useCardStatePreservation<SessionContentBag>({
...
- **`CardStateBag`** — The flat per-card preservation envelope. Optional members compose by axis: `scroll` (outer host-content scroll), `content` (the `useCardStatePreservation` payload), `formControls` (record of `FormControlSnapshot` keyed by `data-tug-state-key`), `regionScroll` (record keyed by `data-tug-scroll-key`), `domSelection` (the card-boundary selection range), `focus` (the `FocusSnapshot`), `components` (the `useComponentStatePreservation` axis, keyed by scoped `componentStatePreservationKey`). Stored in DeckManager's in-memory cache and durably under tugbank's `dev.tugapp.deck.cardstate/{cardId}`. ([D49], [D50])
...
7. **Cross-workspace card move.** A card moved to another workspace leaves the layer its pane stood in and is stood back up in the destination's, so that pane is rebuilt whichever of the two workspaces was the one on screen — a *mounted* workspace's panes are live React trees even while the canvas hides them, so "neither side is visible" is not "nothing renders". `DeckManager.moveCardToSpace` therefore fires `invokeSaveCallback` unconditionally for every card in the moving pane, under the source tag `"space-switch"`, so the bag is there for the mount on the far side; a card whose host is not mounted has no callback registered and the call costs nothing. Like moment 4 and unlike a close, nothing fires `cardWillBeginDestruction`: the card keeps its id, its binding and its session, and only its pane is rebuilt.

   **A workspace *switch* is not on this list, and used to be.** It ran the same iterate-and-save body under the same tag, on the reading that every card in the outgoing workspace was about to unmount and [L26] — do not tear down what is logically continuous — had to be answered rather than obeyed, the answer being that holding every workspace's panes mounted-but-hidden was a cost the design rejected. The measurement behind [B06] refuted the premise: the teardown and the rebuild *were* what a switch cost, and paying them for a workspace the user is only looking away from buys nothing. So a visited workspace now stays mounted and `DeckCanvas` hides it with `display: none`, exactly as `CardHost` has always hidden a pane's background tabs. Nothing is torn down, so there is nothing to capture and nothing to replay, and `activateSpace` runs neither. What survives is the workspace's parked *record*, which persistence and the Workspaces card read — a copy, not a capture.
...
3. **Tab-close-then-reopen.** Same shape as cross-pane move — a fresh `CardHost` mount with a previously-saved bag.
   A **card arriving from another workspace** is this same shape and not moment 1: the card's second mount inside one process run, replaying the bag capture moment 7 wrote. The one-shot `hasAppliedContentRestoreRef` guard is per-`CardHost`-instance, which is what lets the replay fire again.

   A **workspace returning** is on neither list. Its cards never unmounted, so there is no second mount and no bag to replay — what comes back is the DOM that was there all along, at the scroll offset, selection and caret it was left at. That is the whole of [B06]'s argument: the cheapest restore is the one that does not happen.
...
- **Content-owning + engine cards** (`bag.content !== undefined` — every session card is one) have **one text-entry surface**: the engine's editor (`tug-prompt-entry` for a session card). A session card carries no `data-tug-focus-key` / `data-tug-state-key` element of its own, so `captureFocus` only ever returns `engine` or `none` for it — and the assembler leaves `bag.focus` absent in both cases. On restore, `applyBagFocus`'s resolver finds no framework-axis snapshot but an engine-managed card, and routes to the engine resolution. The resolver covers the engine case from the registry tag rather than from the bag; there are no transient in-card framework-axis targets to preserve.
...
- **`deferred-engine`** — engine-managed card whose engine has not yet registered hooks (dev's editor mounts late after `feedsReady`). The dispatcher returns `"deferred"`; CardHost's `subscribeEngineHooksChange` listener re-fires `applyBagFocus` when the engine registers. This is the **one** late-mount focus retry path.
...
- **`transferFocusForActivation`** in `focus-transfer.ts` — intra-pane tab switches (`tug-pane#performSelectCard`), cross-pane activations (the gesture interpreter, via `pane-focus-controller`), tab-close handoffs (`deck-manager#_removeCard` / `_closePane`), runtime chain actions wrapping `activateCard` (action-dispatch.ts `focus-pane`, deck-canvas.tsx `show-component-gallery`). Save outgoing, commit mutation via `flushSync`, gate via `FocusManager.mayClaimActivationFocus`, dispatch.
