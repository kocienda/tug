...
*The lane's conversational entry point is the bare `/dash`, which sizes an idea with the user and routes it to whichever path fits. Typing the stage you want — `/tugplug:poke`, `/tugplug:dash-implement` — is the expert path and is never worse (`/tugplug:dash-devise` is not among them: it is a stage of `/dash` and stops when it is run outside an arc); `/dash` exists so that knowing the roster is not the price of starting. Whichever door a run comes through, the discipline below is the same one.*
