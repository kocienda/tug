...
**The implement and audit stages raise no dialog at all**, and both say so in their tools rather than only in their prose. The refused ledger edit used to be the one exception, on the grounds that a wrong guess there corrupts the durable record — and that reasoning was right about the hazard and wrong about the answer. The hand-edit the dialog offered was itself the corruption; what closes the hazard is a verb for every move (`step reset` to park, `step reopen --why` to un-finish) and `dash doctor` to name which record disagrees. A stage with the right verbs has nothing left to ask.

**The devise and review stages may still raise a design question**, because their product *is* a settled document and a `[Q##]` that was never asked is worth less than a course that paused. It is a genuine cost — a course sits while a dialog is open, in front of whoever happens to be watching — and it is the one place the never-ask boundary is a judgment rather than a rule.
