...
**Whose diff, though.** "The working diff" is the wrong input the moment two sessions share a checkout, which is the normal case here: `git status` is every concurrent session's work at once, so a session that touched one Rust file gets handed the union of everyone's edits and runs tests that neither affect it nor are affected by it. So the selector asks the changes ledger instead — `tugtool changes --json` — and selects from **this session's** classification of the working tree:

- **attributed** — files this session holds a proof row for. Always selected.
- **unattributed** — files no session claims. Selected only when the entry carries a this-session hint (`origin` is not `none`), because an unattributed file with no hint at all is as likely to be the user's own hand-save as anything the session did.
- **foreign** — files another live session holds a proof row for. **Never** selected; they are that session's to test, and pulling them in is precisely the over-broad selection this replaces.

The ledger is an input, not a gate. When it cannot answer — no `TUG_SESSION_ID`, an unresolvable session, no built `tugtool` — the selector falls back to the whole working tree and **says which fallback it took and why**, because a selection that silently changed its own meaning is worse than one that refused. It also refuses to read an empty classification over a dirty tree as a clean tree: that combination is the signature of a `--project` the ledger spelled differently ([L29]), and left unannounced it would select nothing, forever, quietly.
...
**The fan-out has a ratchet.** `MAX_SELECTED` bounds what one selection costs. Alongside it, `ACCEPTED_FANOUT` records the hub paths whose real coupling already outruns `MAX_SELECTED`, and `--check` compares each entry against the same entry in the *committed* file: a number may fall, never rise. Otherwise the edit that widens a hub is the same edit that raises its own ceiling, and the widening never has to be argued. A genuinely wider hub is accepted by deleting the entry and re-adding it — a new key is exempt — so the decision shows up in the diff as a decision rather than as a changed digit.

**Declared surface is not asserted rule.** It is tempting to hunt corpus redundancy by comparing `@covers` sets — a test whose declared surface is a strict subset of another's looks like a duplicate. It is not a usable signal, and a pass over every such pair found nothing to retire. `@covers` answers "which diffs should wake this test," so two tests over one file share a declaration whenever they share a *file*, which says nothing about whether they share a *claim*. What the pairs actually turned out to be, over and over, was a symptom test and a mechanism test on one surface, or two distinct fault classes on one component — and the corpus says so out loud, in docblocks that open by naming the sibling they divide the work with. Redundancy has to be judged from what a test asserts, which means reading it; there is no set operation that stands in for that.
...
## A run is recorded, and a red file arrives with its history {#results-are-recorded}

The runner used to forget everything the moment its shell exited. Every summary it printed — which files were green, against which commit, on which tree — was computed, rendered once, and thrown away. So the question a red file actually raises, *was this already broken before I touched it*, had no cheap answer, and the expensive one is bisection: the `at0417` incident cost roughly thirty-five minutes of reverse-patch probing at about six minutes a cycle to establish a fact that had been true and observable at the end of every prior run.

So the runner remembers. Each run persists what it already computed into `apptest_results.db` — machine-global beside `changes.db`, keyed by the **resolved base checkout** so an arc worktree and the checkout it forked from share one history. That keying is not a detail: the incident lived precisely in that split, with the failing run on an arc and every exonerating run on `main`. History keyed by raw working directory would have answered "no history" at the one moment the answer mattered.

Three properties are doctrine, not implementation.

**The ledger informs; it never gates.** Nothing is re-run, quarantined, skipped, or deprioritized because of what the record says. Selection policy is unchanged, and a known-red file still runs and still fails. The reader decides what the history means; the tool only refuses to withhold it. Recording is telemetry all the way down — a missing binary, an unopenable database, a verb that exits non-zero each produce one named line on stderr and leave the run's verdict and exit code untouched. A test runner that failed because its diagnostics failed would be a worse tool than one that never recorded anything.

**The answer arrives attached to the failure.** A history you have to go ask for is a history nobody asks for, so it is not a query surface — it is a line under the file's name in the `Failures:` section, present on every red file and absent everywhere else. The same object rides the JSON document, formatted from the same lookup, because two renderings of one fact that can disagree eventually will.

**The record says what it actually knows.** An interrupted run leaves no row, so counts are of *recorded* runs and say so in that word rather than implying a completeness they cannot have. A green captured against a dirty tree names a commit whose bytes are not quite what ran, so it is labeled `dirty tree` instead of being flattened into a clean green. And the lookup runs *before* the current run records itself, so a red file's history is what came before it rather than an echo of the failure being asked about. Retention is bounded — the most recent 500 runs per checkout, pruned at record time — because this is diagnostic telemetry whose value is recency, and an unbounded diagnostic ledger is a maintenance task waiting to be forgotten.

The three answers, and what each licenses: **last-green** (the file passed when last recorded — a failure now is new), **red-streak** (already failing, back to a named commit, with the green before it when there is one), **no-history** (never recorded here; says nothing either way).
...
| **Native input / app activation / key window** | **not divisible** — these are login-session singletons, so they are serialized, not namespaced: the whole `just app-test` invocation runs under `tugtool host gate run --name apptest` | `tugcore::ports::APPTEST_GATE_PORT`, `tug::commands::gate` |
| Repo universe (which checkout arc verbs resolve) | `TUG_REPO_UNIVERSE`, exported by the recipe as the invoking checkout — see below | `tugtool_core::find_repo_root_from` |
...
Unscoped, that hop puts a run's fixtures in the wrong universe. A fixture arc created from a worktree is created against the base checkout, while the app under test has the worktree open as its project: the lane can never list the arc its own fixture just made, and the run leaves branches, worktrees and arc-log lines in somebody else's checkout. That is why the corpus used to refuse to run from a worktree at all.
...
### A fixture arc lives in a scratch repository {#fixture-arcs-are-scratch}

**An arc is for implementing a plan, not for running a test.** An arc is a real branch and a real worktree; a fixture that cuts one in the developer's checkout leaves litter in the tree somebody is working in, and a test killed mid-file leaves it there permanently. Fourteen files did exactly that until 2026-08-20, which is why the `app-test` recipe carried a fifty-line janitor sweeping `tugdash/at04??-*` branches and their worktrees before every run.

The rule is now structural, in three places that must stay in agreement:

- **The fixture refuses.** `createArc`, `commitRound`, and `discardArc` in `arc-fixture.ts` throw when the target repo resolves to the checkout under test. There is one constructor for a fixture repository — `makeArcScratchRepo` — and `makeJoinScratchRepo` is built on it, so there is no second implementation to drift.
- **The server hides.** An app-test instance composes no arc entries for the universe checkout (`dashes_hidden_for`, `feeds/changeset.rs`). The `--source-tree` bootstrap makes the checkout a workspace in every instance, so without this the developer's own arcs would still reach a fixture's lane. Session entries are deliberately untouched: the changes-attribution tests compose the checkout's dirt for real.
...
A scratch repo is invisible to the app until a session is spawned on it: tugcast registers one workspace at startup and every other comes from `spawn_session`, so `bindSession` — a client-side binding the ledger knows nothing about — leaves the repo unregistered and its arcs uncomposed. `seedScratchSession` writes the transcript that `spawnSessionResume` opens. The how-to is in [tests/app-test/README.md](../tests/app-test/README.md#a-fixture-arc-lives-in-a-scratch-repository-never-in-your-checkout).
...
### A backgrounded window is fine; a covered one stops painting

Background is not the hazard — **occlusion** is. In pid mode `AppDelegate` sets `window.level = MainWindow.harnessBackgroundLevel` and calls `orderBack(nil)`, which is correct and is exactly what lets a run happen around you. But a window parked behind everything can end up *entirely* covered by whatever else is on screen, and WebKit suspends `requestAnimationFrame` for a view it considers occluded. Event delivery is untouched — `postToPid` never consults z-order — so keys and clicks keep arriving at an app that has stopped painting.

Every wait on a paint then hangs. `harness-smoke/smoke-native.test.ts` sets `window.__tugSmokeSettle` from two nested `requestAnimationFrame` callbacks, so `waitForPaintSettled` never returns and all five cases die on a timeout. Measured 2026-09-03 with no code change at all: **0/5 covered, 5/5 uncovered**, with wall time falling from 47s to 8s because the failures were pure timeout. Anything that reads back a programmatically assigned scroll offset fails the same way and for the same reason — the view never lays out, so the offset never sticks.

**The failure wears a permissions costume, and the tell is what stays green.** DOM-only work is unaffected, so persistence tests and `setTugbankValue` round-trips pass while every native and paint-dependent file goes red, and `AXIsProcessTrusted` answers true throughout. A genuinely missing Accessibility grant does not look like this: it throws `AccessibilityPermissionMissingError` by name, naming the bundle id. So read a settle timeout as occlusion first and look at what is sitting on top of the window, before suspecting TCC, the code signature, or the bundle identity. Getting that order wrong once cost a long detour and wrote a streak into `apptest_results.db` that read like a code regression and was a window.
...
The lifecycle tests drive `NSApp.activate`/`deactivate`, and foreground-tier gestures still post via `CGEvent.post(tap: .cgSessionEventTap)` — keyboard focus, the frontmost window, and screen-coordinate clicks belong to the *login session*, not to any instance. No amount of per-instance namespacing makes two concurrent **screen-taking** runs safe; serialization is physics. The `app-test` recipe therefore re-execs its entire body under a machine-wide gate: `tugtool host gate run --name apptest --label <wtslug>`.

**That argument covers the `@foreground` tier, and only it.** For a long time the recipe applied it to the whole corpus, which is a different and much more expensive claim — the same shape of mistake as selecting from the whole working tree because one session's diff is hard to isolate. A background test posts its keys with `CGEvent.postToPid(ownPid)` and rebuilds its mouse events as in-process `NSEvent`s; it never activates, never takes the key window, and owns a UUID `TUG_INSTANCE_ID` from which every socket, data dir, tmux session and registry entry derives. Two of them have nothing in common to contend for, which is what `launchTugApp`'s own comment has said all along.

So the runner partitions rather than serializes: **background files run `TUG_APPTEST_JOBS` at a time (default 4), screen-takers strictly one at a time, and the invocation gate still serializes whole runs against each other.** Measured on the core tier: 103s serial → 59s, repeatably, with the same 29/29.

Two things had to move for that to be true, and both are the sort of thing that is invisible until concurrency exists:

- **The straggler reap is prefix-scoped, so it runs between batches and never inside one.** `tugtool host instance stop` over `${TUG_APPTEST_ID_PREFIX}-*` reaches every concurrent file's app as readily as its own; called per-file, the first file to finish SIGTERMs every app still under test. The symptom is a batch of unrelated timeouts with one `exitCode=143` buried in it.
- **Timeout budgets travel with the contention.** Every default budget — the RPC per-method defaults, the launch handshake, bun's own per-test timer — is sized for a file that has the machine to itself. `TUG_APPTEST_TIMEOUT_SCALE` multiplies them by the job count; at 1 it rewrites nothing, so a serial run is byte-identical to what it always was.

**The ceiling is 4, and the thing that sets it is a bug rather than the hardware.** At 6 and above every file fails on its *first* `evalJS` — `evalJS("1 + 1")` included — with "returned a result of an unsupported type". That is the launch handshake reporting ready before the WebView can evaluate: a readiness race whose window is only wide enough to lose under load. Raising `TUG_APPTEST_JOBS` means fixing that race first; the number itself is not the constraint.
...
| Notify socket `tugbank-notify-<token>.sock` | tugcast | `bind` at boot | `remove_file` on graceful shutdown (`tugcast/src/main.rs`); crash residue by `tugtool host sweep` (connect-probed, live-namespace and age gated) | Crash → stale file until the next sweep. `$TMPDIR` does **not** reap these — believing it did is how 9,833 accumulated |
| Control socket `tugcast-ctl-<token>.sock` | tugcast (path from Swift) | `bind` | ProcessManager unlink on close; crash residue by `tugtool host sweep` | Same as notify socket |
| **tmux server** `tug-<token>` (daemon — *outside* any process group) | tugcast (`-L`) | first `tmux` call in the terminal feed | app-test: self-reap on shutdown (`main.rs`, gated on `apptest-` id); dev/release: `tugtool host instance remove`/`prune` and `arc join`/`discard` (`reap_instance_tmux`); `tugtool host sweep` at both ends of the recipe and at dev/release startup | Covered on every managed path; the sweep is registry-anchored rather than slug-scoped, so it reaches orphans a deleted worktree left behind |
...
| Gate port (`APPTEST_GATE_PORT`, 55600) | `tugtool host gate` holder | `TcpListener::bind` at invocation start | OS on process exit — including SIGKILL (`FD_CLOEXEC` keeps gated children from inheriting it) | None — bounded to process |
| Per-worktree app-test DerivedData (`Tug-apptest-<wtslug>`) | `just build-app` (forced identity) | first app-test build in a worktree | **nothing automatic** — `just clean-all`'s `Tug-*` glob, or manual `rm` | The known leak: an arc that ran app-test leaves a DerivedData tree behind after `arc join`/`discard`. Disk, not correctness. Deliberately outside the janitor: it is build output, not runtime debris, and no probe or registry entry can say whether an Xcode build is mid-flight |
| Instance data dir (`instances/<id>/`) | tugcast | `write_bundle_path_marker` + tugbank/sessions/Logs at boot | harness `wrappedKill` → `instance remove --data-only`; app-test residue by `tugtool host sweep` (registry + marker + age gated); bundle-missing dirs by `tugtool host instance prune` | SIGKILL or a launch failure skips the harness path; the sweep is the backstop. Dev/release dirs with an intact bundle are never candidates |
| `$TMPDIR` test artifacts (`tugcast-test-changes-*`, `tugapp-test-tugbank-*`, `tug-scratch-*`, …) | the tests that mint them | per test / per spawn | the creator's own teardown (TempDir drop, `rmTempTugbank`, `testTmpDir`), then `tugtool host sweep` at 24 h for anything a SIGKILLed runner left | Every prefix must be registered in `tugcore::janitor::TMP_PREFIXES` — `no_unregistered_tmp_prefixes` fails the build otherwise, so a new debris class cannot be invented unswept |
...
- [`tugrust/crates/tugtool-core/src/apptest_ledger.rs`](../tugrust/crates/tugtool-core/src/apptest_ledger.rs) — schema, base-root resolution, `record_run`, `file_history`'s three answers, and the retention constant.
- [`tugrust/crates/tugtool/src/apptest.rs`](../tugrust/crates/tugtool/src/apptest.rs) — the `tugtool apptest record|history` verbs, the recipe's only way in.
