...
**And two registers, which is a different axis from scale.** Both scales above are the TRACK register, for a surface whose subject is the arc. Where a SESSION is the subject — the masthead's title run, the Cards card's session rows — the row wears `ArcLifecycleMark` instead, and never the track ([D168]): a strip beside an eliding name is a graphic competing with the thing the row is named for.
