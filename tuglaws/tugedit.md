...
`tugedit --preview` runs phases 1–3, then prints the unified diff the program *would* produce and exits 0 without writing. This is the model's dry run and it should be the reflex before any program with `all`, a regex, a `files` block, or a `move`. Preview emits no receipt and touches no mtime, because it writes nothing at all. [`file probe`](tracking-changes.md#verb-receipts) reaches the same end — nothing changed, so the ledger must not say otherwise — by the opposite means: it *does* write, twice, so it advances the mtime honestly and **declares** the restore (`{"ops":[],"restored":[…]}`) for the relay to drop the bracket row. Rewinding the mtime instead is what left builds trusting stale artifacts.
