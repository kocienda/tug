...
| `resize-sidebars-to-fit` | **⌃⇧⌘S** | The counterpart of a Tug-tier command, on the ⌃⌘S key itself: the same subject, the opposite thing to do with it. Beside Disclaim All ⌃⇧⌘A, which stands against Claim All ⌃⌘A the same way. |

**The resize took two dead seats before this one.** It held **⌃⌥⌘R** — a grant with a reading rather than a derivation, R for *Resize*, because ⌃⌘R is the Arcs card and no variant of it — and then **⌃⌥⌘S**, when Hide Sidebars taking ⌃⌘S gave the ⌃⌥⌘ tier a base for it to be the advanced form of. That derivation was sound and the chord was still dead: Keyboard Maestro holds both keys on ⌃⌥⌘, so neither press ever reached Tug and the Window row was the verb's only door. ⌃⇧⌘ keeps everything the second reading earned — the subject, the key, the pairing with ⌃⌘S — and trades *advanced form of* for *counterpart of*, which is the truer reading anyway: hiding a rail and fitting one are opposite things to do with the same object, not one taken a step further. See the reservation below.
