...
⌘M Minimize → ⌥⌘M Fold Session was the *form* half of that rule rather than the verb half, and the derivation was sound: ⌘M puts the window away and keeps it; ⌥⌘M puts the card's body away and keeps it. The two verbs differ because the objects do — a window leaves the screen for the Dock, a folded card stays on the wall reporting — but the gesture is the same shape aimed at the smaller object, which is what this tier is for. The key was ⌘M's rather than an F-shaped one because every F is spoken for: ⌘F is Find, a ⌥ twist of it has to read as a variant of opening the find bar (see the ⌘F entry below), and ⌃⌘F is Full Screen.

**The derivation landed on a chord Tug cannot bind.** AppKit synthesizes Minimize All as the ⌥-alternate of Window ▸ Minimize, so ⌥⌘M resolves at the system's menu layer before Tug's Session menu ever sees it — the same situation as the ⌃⌥⌘ reservation below, and a chord that never arrives is indistinguishable from a command that does not exist. The paragraph above stays because its *why not F* argument is still true and is what stops ⌥⌘F being re-proposed; what it no longer describes is where Fold lives.

**Fold Session is ⌃⌘Y**, in the Tug tier, where the Session menu's own ⌃⌘C and ⌃⌘H already sit. A base tier takes a reading rather than a derivation, and this one's reading is the **glyph**: a Y is a fold drawn — two arms meet and continue as one stem, which is the open card's two regions, transcript and composer, closing down onto the one bar the folded card is. Every other letter in this tier is an initial (C changes, H history, I AI, U usage); a shape is a different kind of reading and is recorded as one so the next reader knows it was chosen rather than left over. ⌃⌘M was not available and was not to be taken: it is Generate a Commit Message, and a `menuEligible` grant there would resolve before the composer's scoped binding could see the keydown.
...
| Set | Reading |
|---|---|
| ⌥⇧⌘ | Both twists at once. "…as Plain Text" is always ⌥⇧⌘ (⌥⇧⌘C, ⌥⇧⌘V — the latter matching macOS "Paste and Match Style" exactly). On the bracket row it is the slot nudge ⌥⇧⌘[/], both twists of the lateral ring; on the arrow row it is free in full, and recorded free rather than spent ([D184]). It also holds the one chord the hand chose rather than the algebra: Resize Sidebars to Fit ⌥⇧⌘S, recorded under "A chord has to be typable" below. |
...
| ⌃⌥⌘ | The variant or advanced form of a Tug-tier command — the "super-advanced" tier. **Unavailable for new grants, and down to one resident.** The set is spoken for outside Tug, recorded below. Cycle Permission Mode ⌃⌥⌘P is what is left, since permission modes govern agent autonomy, the quintessential expert feature; it is the tier's debut resident and now its only one. Resize Sidebars to Fit sat here, on ⌃⌥⌘R and then ⌃⌥⌘S, and left when both were found to be keys the tier's outside claimant already holds. The tier briefly held the Changes shade's bulk verbs; [D175] returned those to the tiers [D126] derived for them. |
...
**One resident is grandfathered** — ⌃⌥⌘P Cycle Permission Mode — and grandfathered is the whole of its standing rather than evidence the set works. It is reachable from its menu row and it is rebindable. A second would not be granted.

**The tier had a second resident until 2026-09-17, and it had never once fired.** Resize Sidebars to Fit held ⌃⌥⌘R, then moved to ⌃⌥⌘S; the claimant holds both. Keyboard Maestro's Global Macro Group carries a `rocket` macro on KeyCode 15 and a `modifier: shift` macro on KeyCode 1, each with `Modifiers 6400` — ⌃⌥⌘ — so every press of either chord was swallowed by a system hot-key registration before Tug's menu bar was ever consulted. Nothing in Tug was wrong: the registry stated the binding, `codeToKeyEquivalent` converted it, and `applyCommandChords` wrote it onto the Window row, where an app-test asserts it to this day. The chord simply never arrived, which is why the verb was reachable only from its menu row and read as a command that half-existed. **Grandfathering is standing, not function** — a resident here works only on a key the claimant happens to have left alone, and ⌃⌥⌘P is that and nothing more. Resize Sidebars to Fit left, and where it landed is the next section.

## A chord has to be typable

**A hand shape the user will not form is as dead as a chord the OS eats**, and the algebra can see neither. The section above is about keys claimed by another program; this one is about the same failure arriving from the other direction — a chord that is delivered, correct, and never pressed, because reaching it is worse than reaching for the menu. Both produce a verb whose only real door is its menu row.

**Resize Sidebars to Fit is the case.** Off dead ⌃⌥⌘S it went to **⌃⇧⌘S**, the counterpart tier, which is where the algebra puts it: the same key as its ⌃⌘S base, the opposite thing to do with a rail, beside Disclaim All ⌃⇧⌘A standing against Claim All ⌃⌘A. The derivation was right and the chord lasted a day — ⌃⇧⌘ on the left of the board is a shape the user will not make. It sits on **⌥⇧⌘S** instead, a **grant with a reading rather than a derivation**: the constant is the *key*, since every sidebar verb is on S, and the set is whichever one the machine and the hand both leave free. The composed-set rule's warning about climbing the modifier stack for want of a free seat is real, and this is the exception it admits, recorded with its reason rather than dressed as a derivation it is not.

**The rule this leaves behind:** propose the derived chord, then ask whether the hand will form it. When the answer is no, say so and grant the typable one *as a grant* — the algebra ranks readings, and the user ranks reaches. R1's ban on climbing the stack stands for every chord chosen for room; this exception is for a chord chosen for the hand.
...
**⌃⌘⟨letter⟩ names a sidebar card, one letter per card.** The two families do not compete: **the pair addresses a SIDE of the deck, a letter addresses a CARD.** The letters are a complete set rather than an open-ended one — the sidebar holds five cards and registration is a boot step, so every card has a letter and there is no sixth to derive a grant for. Three of the five are initials; Cards and Arcs are not, each for a reason the table gives.
...
**A chord per side does not grow at all**, which is the pair's own argument: five sidebar cards need the same two keys three do, and two is closed because the deck has two edges. The letters are closed for a contingent reason — the card set is five — where the pair's is structural, so **a sixth sidebar card is the letter family's to answer for.**
...
**These are `menuEligible` with empty Swift key equivalents** (Go ▸ Go to Slot *n*), so `applyCommandChords` writes them and all six stay rebindable — the discipline the rail pair follows, and the one the shade toggles below do not.
...
**Unpromoted, deliberately.** The nudge acts on the *layout selection* — a fact about the Cards card's list, not about the frontmost card — and a mirrored `validate` is computed when the menuState is pushed, which a selection change does not push. A menu item would therefore be permanently enabled and intermittently inert.
...
| `resize-sidebars-to-fit` | **⌥⇧⌘S** | A grant with a reading rather than a derivation ("A chord has to be typable"): the key is the constant — every sidebar verb is on S — and the set is the one the machine and the hand both leave free. ⌃⇧⌘S is where the algebra puts it; the hand would not form it. |

**The resize took three seats before this one, and none of them worked.** It held **⌃⌥⌘R** — a grant with a reading, R for *Resize*, because ⌃⌘R is the Arcs card and no variant of it — then **⌃⌥⌘S**, when Hide Sidebars taking ⌃⌘S gave the ⌃⌥⌘ tier a base for it to be the advanced form of, then **⌃⇧⌘S**, the counterpart reading, which is the truest one the algebra offers: hiding a rail and fitting one are opposite things to do with the same object rather than one taken a step further. The first two were eaten by Keyboard Maestro and never reached Tug at all; the third arrived and went unpressed, because ⌃⇧⌘ on the left of the board is not a shape this user's hand makes. What survives all three is the **key** — S, with ⌃⌘S — and ⌥⇧⌘ is what the machine and the hand leave. See the two sections above: one about keys claimed from outside, one about shapes the hand declines.
