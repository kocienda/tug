...
**A slot resolves two ways, and which one is the deck's to choose.** `imposition.layout` is `"fit"` or `"flow"` (absent reads as fit, so every deck written before the mode came back exactly as it stood), written by the Lens Layout section's `set-imposition-layout` action. **Fit** is the travel-fraction rule above. **Flow** reads the same slots as ordinal positions in a strip: the occupied slots stand side by side in index order, each taking its own extent — its widest member's width — one imposition gap apart, so `stripLeft(k)` is the running sum of everything before it. An unoccupied slot contributes nothing, not even a gap; a strip is a sequence and an empty slot is not a member of it. `kind` still names how many slots there are and still answers ⌘1..9 and the miniature; only what a slot MEANS changes.
