...
**A corridor drag reorders on half-overlap: the threshold is the crossed member's own middle, and what crosses it is the dragged frame's leading edge** — its bottom going down, its top going up, both read against the tiles the members held when the drag latched. Resting geometry is what the eye is reading and what keeps the predicate stable and monotone in the pointer delta; the siblings' live preview positions are the answer being computed, so consulting them chases its own tail. Two near neighbours are wrong. Measuring against the run the siblings *would* take with the dragged member lifted out pulls every one of them up by its whole height, which on a two-member rail puts the crossing above the dragged card's own resting middle — the order flips on the first pixel of travel. And middle-against-middle fires, for members of different heights, only once the dragged card has gone **past** the place the swap will put it, so the shuffle snaps backwards under the hand; half-overlap always fires short of the destination, by the same fraction in either direction, whatever the two heights are. Which edge leads is decided by resting position, not by the sign of the delta, or a wobble at the crossing would swap the leading edge and toggle the order.
