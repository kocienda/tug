...
| `tugdeck/src/components/chrome/flow-rail.tsx` | `FlowRail` — where the deck stands in its flow strip, and the thumb, segments, and wheel that move it |
| `tugdeck/src/components/chrome/flow-rail.css` | The rail's two registers: quiet while the strip fits, raised with a thumb while it overflows |
