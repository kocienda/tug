...
**A card is dragged to a place, not to a coordinate.** Dragging an arrangeable card by its title bar moves it freely under the pointer, and throughout the gesture exactly one advertised **drop zone** is indicated — initially the card's own current position, which is what makes a zero-travel release a no-op rather than a question. The indication snaps between zones as the pointer moves, and the release animates the card into the zone that was showing and commits that zone's mutation. Because a zone is always live, an unmodified release is always meaningful: there is no "released over nothing" to invent an answer for. A zone's rect is the tile the card would occupy if released there, derived under the standing the target column would have AFTER the drop — so a two-member column about to take a third draws its `run / 2.5` strip rather than its shares, and the indicator cannot promise a geometry the commit will not produce.

**A zone is asked for where its members are, and lands where its tile is.** Those are two rects, and they come apart wherever the tile a card would land in is a poor description of the region that should select it. A split column's positions are asked for at the **members**: the run divides at the midpoint of each card standing there, so the top member's upper half asks for position 0, the bottom member's lower half asks for the last position, and every position is a band the eye can find. Choosing by the landing tile instead makes the last position of a column about to overflow all but unreachable — its tile begins four fifths of the way down a run it then hangs off the bottom of, so only a pointer in the run's final sliver could ask for it, and a full-height card dragged anywhere over the bottom member read as "between the two". Rails divide the same way, for the same reason. The tile is untouched by any of this, so the indicator keeps its promise: what it draws is still where the release lands.
