...
**And a rotation ends the retiring claude's turn as a cancel, never as an error, so the card wears no banner for it.** The rule above makes an open turn rare rather than impossible — a turn the dispatcher never intercepted reads idle while claude is working, which is the same wrong reading it refuses to act on — so the bridge closes whatever it finds as `turn_cancelled` with `is_recovery` before the kill, and the deck commits that turn quietly: `handleTurnCancelled` ends it as interrupted, and its end-state badge reads "Recovered" in the surrounding text colour rather than claiming the user stopped anything. An `error` frame is the one message that locks a card body behind a "Protocol error" banner, and a rotation is the wheel's deliberate act: the turn it retires was not lost, and a banner raised on a line the rotation has already closed has no path off it.

That paragraph used to end by calling the cancel "the frame family the deck already renders quietly", and that was not true when it was written. The deck had no `turn_cancelled` entry in `KNOWN_CODE_OUTPUT_TYPES` at all, so the ingest guard dropped the frame with no dispatch and no log: a rotation raised no banner, but it also settled nothing, and the same silence left an escalated Stop reading "Interrupting" with nothing able to end it. The handler is what makes the claim hold, and it was written by the network-resilience arc.
...
**An arc comes in two kinds, and the record decides which.** Both open on a brief. A **planned** arc runs devise → review → implement → audit; a **plain** one runs implement → audit, because the `/arc` door's whole economy is skipping the settling those two stages buy. The kind is derived from the arc's documents by `open_arc` when `tugtool arc run` opens it, written into the arc's durable record as an `arc-kind` line, and read from that record by `start_action` ever after. Derived once, recorded once: the documents answer the question at the opening and the record answers it for the rest of the arc's life, so nothing downstream can be told a kind the documents contradict.

A **task list beside the brief is what makes an arc plain**; a brief alone, or a plan, makes it planned — `plan.md` outranks `tasks.md` here exactly as it does everywhere else. That is the door's whole hand-off: the settling it did or did not do is legible in what it left on disk, so no flag has to carry the fact separately. Within a planned arc the documents still say how far along it the arc already is: a document that lints as a plan opens at review, a brief alone at devise. That derivation is exactly the one every arc had before the kind was recorded, so an existing arc resumes unchanged.
...
**Like a rotation, a compaction happens at a turn end and never inside one.** The turn-end rule above is the whole reason: a prompt sent into an open turn would queue behind a model still working, and there would be no idle edge to read the result against. Under an arc, the implement stage therefore closes one step per turn and ends it, so every step boundary is a turn boundary the arc can act on — a rule of the stage's ask and its skill rather than of the wheel, because only a model can end a turn. **The compaction's own end is a turn end like any other**: the `/compact` the arc sent closes a turn, that close is held for the settle exactly as every other idle edge is, and the stage is prompted on from it — judged against a reading taken after the compaction or, until one arrives, against none, because a compact turn carries no usage frame of its own and a missing reading is no reason to strand a stage with steps left. This shape is not a stop and must never become one.
