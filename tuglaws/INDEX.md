...
- [menus.md](menus.md) — The macOS menu bar as a projection of the command registry: the `menuState` wire contract including the per-item `commands` gate, the mirror-first validation model, the four-layer chord resolution order, the generated command catalog and chord table, and the identifier namespace. Also the context-menu doctrine — one entity, one menu, from the annotation registry: the registry states the items and the surface states its facts, the fixed order, the copy-noun vocabulary, and when the standard editing block appears.
...
- [list-surface-grammar.md](list-surface-grammar.md) — Which component paints a section header (`TugSectionLabel`), a row's run of small facts (`TugMetaRun`), and a dash's name (`TugDashAtom`); the two registers a dash is named in and why the register itself carries the fact; the atomic-parts rule; the optical outdent for a pill that leads a row; and the vocabulary (`uncommitted`, no possessives). Read before adding a surface that lists files, dashes, or sessions.
...
- [dash-lifecycle.md](dash-lifecycle.md) — What a dash *is*: the four things it is made of, the owner-key identity model, the seven stages and the derive-vs-declare rule that assigns them, what a binding means, and the faces a dash wears. Read this for "why does this dash read as parked"; read the doctrine below for "may I write here".
- [wheel.md](wheel.md) — The layer that seats a claude session under a card. What a rotation is, the three kinds of carried thing (invariants, parameters, always-dropped) and why the invariants are expressed as absence, the turn-end rule and why there is no perform-at-request-time path, how a course ends and hands the card back, what survives a relaunch, and the three faces (`POST /api/session`, `tugutil session rotate`, this doc). Read before changing how a session is seated or retired.
...
- [devise-skeleton.md](devise-skeleton.md) — Template for plan documents (the format `/tugplug:dash-devise` authors and `/tugplug:dash-implement` walks). Kept here per user decision; it is a template, not a tuglaws law or architecture doc.
...
- [dash-review-rubric.md](dash-review-rubric.md) — What a reviewer judges when reading a plan, and what belongs to `tugutil plan lint` instead. Read by `/tugplug:dash-review`.
