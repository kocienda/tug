...
| `claim` | proof | `Claim` | A session explicitly claims paths (`tugutil changes claim` → `changeset_claim`) | On the claim request; records one `whole` span — a claim is a whole-file assertion by hand |
...
**Ownership is renounceable.** `changeset_disclaim` (`tugutil changes disclaim`, and the per-row control in the Changes shade) deletes **all** of the requesting session's rows for the named paths — proof and bracket alike, because leaving the session's own bracket rows behind would go on hinting `likely` about a file it just gave up. It is the inverse of a claim and deletes only the requester's rows: another live owner becomes sole owner, and with no other owner the file degrades to `unattributed`. That makes it the one-click resolution of a contended file in the other party's favour, and it follows the same degradation direction as everything else here.
...
The workflow layer over the soundness axioms above ([D116]): every change lands through one lifecycle — **Preflight → Draft → Land** — in two lanes (main-lane commit, dash-lane join), one room (the Changes shade), one landing gesture (the prompt-entry verbs `/commit` and `/dash-join`).

**One landing mode owns the composer, and there is one slot.** Commit mode and join mode are the same shape — one `LandingMode` interface (`tugdeck/src/lib/landing-mode.ts`), two controllers, one `landingMode` slot on the prompt entry — so mutual exclusion is structural rather than remembered: entering join mode exits commit mode because the slot can only hold one thing. The composer's route group (Z4A) shows which lane is up, and its third segment appears only while the card is bound to a dash. The composer's document *is* the landing message in both modes; a mode never renders its message read-only somewhere else.
...
**The dash lane fronts the outcome, and the act that clears it.** A join is the one landing that can be refused for reasons the user cannot see, so the lane asks before offering: opening the card's own dash row spends a `--preview` and the row wears the answer. Five outcomes, and every one of them names what to do next:
...
**Receipts are transcript ink.** A successful landing appends a non-context row to the session's transcript — verb, short sha, subject, counts; joins carry their dash provenance; releases name what was discarded. Every summary is formatted **once, on the server** (`format_commit_summary` / `format_join_summary` / `format_release_summary`), written to the shell ledger, and returned on the reply: the live row and the row a Maker ▸ Reload restores are the same bytes through the same parser, and a row the parser does not claim renders its raw output rather than nothing. History badges join commits by the `Tug-Dash:` trailer already on the squash message.
...
| Changeset card / feed | `feeds/changeset.rs`, `feeds/changeset_all.rs` ([D113]) | Composes live ledger rows per project (same liveness rule); resolves multi-owner paths through the hunk-aware verdict and marks `shared` only on true overlap, carrying each owner's `own_hunks`/`contested_hunks`; the card's default selection is every attributed file and **OFF for unattributed** (inclusion is an explicit per-file election — the card mirror of the exit-3 refusal), and on a contended file the default *election within* the file is this session's own hunks |
...
| `tugutil changes disclaim` | `tugutil/src/changes.rs` → `changeset_disclaim` | The inverse of a claim: drops the requesting session's rows for the named paths (see the read side); the deck's per-row control and the shade's bulk button are the same verb |
