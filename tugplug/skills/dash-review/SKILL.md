...
This computes the plan's content stamp and inserts `Reviewed \`plan:<hash>\`.` into the round you just wrote. From then on `tugtool plan status` can say whether the review still covers the document, which is what `dash-implement`'s setup gate reads.
