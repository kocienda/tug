...
**Every ledger gesture from here on announces itself on the card**, written by the verb rather than by whichever stage happens to remember: the dash created, the run declared, each step opened and closed, each round committed. That is what the user watches a run by, so it is machinery rather than a stage's manners — but if you are ever on a build where the lines do not appear, say each gesture you make in one line yourself, in the same shape (`<dash>: run declared through step m of N`), before anything else in the reply.
