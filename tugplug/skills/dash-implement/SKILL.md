...
**And the boundary is machinery, not only these words.** It was only words once, and on the course machinery's first live run a stage walked step 1 correctly — open, work, commit, close — and then kept going straight into step 2 in the same turn, through this paragraph and the wheel's own opening prompt alike. So two things now hold it up. Every step verb's output ends with the sentence naming what the discipline demands next: a close says *End your turn now — the course prompts Steps N+1–M*, an open says *This turn closes step N and nothing else*. And the PreToolUse gate refuses the overrun outright — once a turn has closed a step, a repo write or a `dash step start` from that same turn is denied, naming the step that closed and the one act that unblocks everything.

**If you meet that refusal, the answer is to end the turn.** It is never to find a spelling the gate does not read: the gate is the discipline, and a stage that routes around it is the incident. Reads, `dash status`, `dash doctor`, `dash commit` and the draft verb all stay open, because reporting the step you just closed is not the next step's work.

**Every ledger gesture announces itself on the card, and that is the machine's job rather than yours.** A dash created, a run declared, a step started, closed, withdrawn, reset or reopened, a `mark`, each round — every one lands a one-line row on the card as it happens, written by the verb server-side. Do not narrate them again in prose: the card already has the line, and a second copy in your reply is the same fact twice. If you are ever on a build where the lines do not appear, say each gesture in one line yourself in the same shape — `<dash>: step i/N closed` — because the person watching the run has no other sign of it moving.
