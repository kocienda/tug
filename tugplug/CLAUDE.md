...
Two kinds of work leave the base on a worktree and come back to it through a join. A **dash** is the documented kind: a brief, a plan, and an **arc** — the server-driven sequence that rotates the card's session through the `devise`, `review`, and `implement` stages, each on the model the project declared for it. A **cut** is the other kind: no document, no review, no arc — a worktree cut from the base, rounds, and the join. The skill names follow the split: the `dash-*` family is the arc's stages, `cut` stands alone, and `dash` is the door to both.

**The dash family:**

- **`dash`** — the lane's conversational front door, invoked as the bare `/dash`. Takes a free-text idea (or nothing), sharpens it, routes it with one question to a cut, a design spike, the plan arc, or a brief-then-plan, and on the arc route writes the brief and hands it to `tugutil dash run`. It **sequences and narrates; it does not restate** — at each hand-off it reads the sibling's own `SKILL.md` and runs that contract in-thread, so the expert skills stay the single source of truth for their own mechanics and every one of them stays independently invocable. Creates no worktree, commits nothing, joins nothing.
