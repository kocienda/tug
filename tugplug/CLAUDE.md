...
- **`dash-plan`** — the planned dash, invoked as `/dash-plan [idea…]`. Orients on what is in flight, sharpens the idea with the user, writes the **plan** in this conversation on the user's own model against the devise skeleton, lints it clean, and hands it to `tugutil dash run` as the turn's last act. It **narrates; it does not restate** — the arc's stages are the single source of truth for their own mechanics. Creates no worktree, commits nothing, joins nothing.
...
- **`dash-review`** — the arc's first stage on a plan: lint the plan (`tugutil plan lint`), judge it against [`tuglaws/dash-review-rubric.md`](../tuglaws/dash-review-rubric.md) and the real code, **apply the fixups in the plan**, append a Review Record, and stamp it with `tugutil plan stamp` as the last edit — which is what lets `tugutil plan status` say afterwards whether the review still covers the document. Reached by the arc, or from a typed `/dash-review` in the card. It runs as an ordinary turn on **whatever model is selected** — nothing borrows, nothing switches. It is never run inline by the session that wrote the plan, on any model: the cold read is the whole point, and handing the reviewer the author's context is what destroys it.
...
- **`tripwire`** — lay, revise, and shake down a **tripwire**: a standing watch that fires on a recorded fact or a commit, answers a brief, and reports itself in the Overview. Rides `tugutil tripwire` and authors nothing else. **Never joins** — a tripwire may stage work on a dash and say so, and landing it is the user's act.
...
The lifecycle skills run in the main conversation and ride the `tugutil dash` CLI (`create` → `step start` → `commit` → `step done` per step, `mark` for the stages git cannot see). The flow starts at whichever door the user typed — `/dash` for work done here, `/dash-plan` for work the arc carries, under which the server rotates the stages itself and nothing needs typing between them. The expert path is to type the stage you want: `/tugplug:dash-review` → `/tugplug:dash-implement` → `/tugplug:dash-audit` → the user's join gesture in the Session card (the working run leaves the dash's join draft behind for it).
