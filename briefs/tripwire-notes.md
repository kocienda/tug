...
- **tugedit failures.** When a `tugtool file edit` program fails to resolve, an AI looks at the failure *right when it occurs*, diagnoses it, and proposes the corrected program.
...
- **No tugbank for tripwire state** — tugbank is per-instance, and tripwire state must travel with the tripwire. Model, pause, cooldown are columns on the tripwire row, read at trip time. The engine re-reads the tripwire set when evaluating a candidate event, so `tugtool tripwire pause ci-confidence` takes effect on the next event with no notification plumbing.
...
A work-tier trip **lays a real dash**. The tripwire's agent is a real headless tugcode session — genuine session id, genuine transcript, spawned through the supervisor beside `spawn_session` via a new cardless entry point — running on the dash worktree, committing there via `tugtool dash commit` under the existing dash commit discipline. The trip's artifact is not a loose patch; it is a dash with commits, an oplog, and documents at `.tug/dashes/<name>/`. Review happens in the Join sheet, landing is the user's `/dash-join <name>`, conflicts arrive as data, and `--resolve` stays a user-gesture-only act. **The tripwire stages; the user joins.** Tripwires only ever commit on their own dash worktree — exactly the exception the git policy already carves out.
