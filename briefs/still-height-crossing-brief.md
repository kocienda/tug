...
**[F02] A column mode flip gives exactly one frame a height term; a split join or leave gives one to every member that resizes.** On a mode flip the z-frontmost member is the **survivor** and rides a fused beat that tweens its real height; every other member is **covered** and animates nothing (`settleHoldPlanRef` doc, `deck-canvas.tsx`). A card joining or leaving a split column is not a mode flip: nothing is covered, `heightTweens` is computed per frame from its own rect delta, and every member whose tile changes height tweens it — so several session cards can be laying out at once. Whether the gesture judders is therefore decided by which frames carry a height term and what each interior costs to lay out — which is the "sometimes". **(verified, by reading)**
...
**[F04] Those re-windows are commits inside the settle window, which the doctrine measured as superadditive, and the existing hold does not cover them.** `animation-doctrine.md` (#residency): settle alone 343 walk samples, a commit stream alone 654, both together 1809. `CodeSessionStore.holdNotifications` defers **wire-origin** notifications only — "Local actions and timer ticks are never held" — so a list view re-rendering off its own `ResizeObserver` lands inside the window regardless. And it commits on **every** delivery, not only on a window change: `scrollTick` is a bare `useReducer` increment (`const [, scrollTick] = React.useReducer((x) => x + 1, 0)`), and the container observer calls it unconditionally after a synchronous `maybePinToBottom()` — a `scrollTop` write against fresh geometry — and `applyRestoreTarget()`. One whole-list-view render per animation frame, for the life of the tween. **(verified, by reading `tug-list-view.tsx`)**
...
**[F12] Under FLIP, the hold is nearly free.** React commits the final geometry before the Last pass runs, so on a **growth** the interior is already laid out at the larger (final) height and the hold merely keeps it — zero extra layouts. It is not zero observer deliveries, though: the frame is held at its First height while the interior stands at Last, so the scroller's size differs from the last one its `ResizeObserver` delivered, and one delivery lands at launch, before the first painted frame — then none until landing, where the size does not change again. On a **shrink** the hold restores the First height in the same pre-paint pass, so a `ResizeObserver` comparing against its last delivered size sees no change at the start; the one real relayout and the one delivery land when the hold comes off, after the tween. **(inferred from the effect ordering; [B07]'s gate is what pins it)**
...
**[B07] Two gates prove it.** An app-test over session cards with long transcripts, in two shapes — a stack and a split of a column whose survivor is one, and a split join that shrinks **two** of them at once ([F02]) — asserting **no** `ResizeObserver` delivery inside a held card after the tween's first painted frame and before its landing. A growth is allowed its one delivery at launch and a shrink its one at release ([F12]); asserting zero outright would fail correct code. And an on-demand release bench of the same gesture before and after, under [D5] discipline (unoccluded, interleaved baselines, ×3 medians, forcing probe). The bench stays out of CI, per the doctrine.
