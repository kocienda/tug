...
**[F09] Neither *Later* nor *Install and Relaunch* does anything, and the confirming observation could never have been read** — reported from use. The leading hypothesis was that `TugUpdateDriver.perform` falls through because the state holds no matching reply closure — the deck drawing a snapshot whose Sparkle session the host has already torn down, so both buttons are inert *and silent*. The line named as the cheap confirmation, `TugUpdateDriver: ignoring '<action>' — state <stage>`, was an `NSLog`, and `NSLog` goes to Console and the unified log rather than to `tugapp.log` — which `TugLog.swift`'s own docblock says in as many words. So no run's `tugapp.log` could ever have carried it, and no such line is in this machine's unified log either. The instrument was the first defect: the refusal path is now recorded through `TugLog` and answered on the surface, so a future report of a dead button has a line to read. **(the hypothesis is neither confirmed nor killed; the diagnosis is that the record it rested on does not exist)**
...
- **How far down from the window's top edge, and does the surface overlap the title bar?** **Settled: the drop the surface already had, `calc(var(--tug-chrome-height) + var(--tug-space-sm))`, which is one small gap below the chrome.** Only the horizontal anchor moved. It clears the chrome by construction, so there is no title-bar overlap to resolve, and it is nothing like `[F03]`'s 22vh — that is a search field the user summoned, this is a notice that came to them. `at0612` measures it at 42px in a harness window and pins it under 15% of the window's height, so a rule that drifts back toward a search field's drop is caught. *Not settled by eye against the running app with the sidebar open and closed*: the app-test is the only reading taken.

- **Does a hidden surface stay hidden across a deck reload?** **Settled: no, and that is fine.** The size is the deck component's own state, so it survives a stage transition — a badge hidden while `downloading` does not come back at `readyToInstall`, which is what makes the `x` worth anything at the stages a user reaches for it — and it does not survive the flow or a deck reload. The host replays its snapshot to a fresh deck and the surface returns. Nothing is asked to remember a hide, which is the point: hiding defers the surface, and a new flow is new news.

- **Is the `x` offered at every stage, or only where deferral means something?** **Settled: every stage.** Hiding `installing`, seconds from a relaunch, is pointless rather than harmful, and a control that appeared and disappeared per stage would be its own small version of the inconsistency this work exists to remove. The badge is one shape wherever it stands.
