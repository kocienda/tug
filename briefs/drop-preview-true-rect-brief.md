...
**[B02a] That rank is the CONTENT card's; a rail card in the air keeps the old one.** Found by [F05]'s drag and answered there. The rule behind [B02] is that the outline lives under the same occlusion its place lives under — and a rail card's places are the rails themselves, which is why [B01] leaves rail zones unclipped. Ranked under the band, a rail drag's outline is painted over by the very panel it is tracing a run inside: a probe carrying the rail's bottom member up drew the outline at 8950 with a sitting member standing over the whole of it at 8992. That is erasure, not occlusion. So `chrome.css` declares two tiers and `[data-carrying="rail"] .tug-drop-zone-indicator` takes the second — 99960, the value the indicator carried before the split, above the whole band and below the snap guides at 99970. `data-carrying` is already written on the indicator's own parent by `deck-canvas.tsx`, so nothing new has to be published.

**[B03] `at0549-card-drag-leaves-the-rail`'s rail pin asserts occlusion, not absence.** The outline's box may cross the rail's near edge; the rail must paint over it. The reading is the **computed `zIndex` comparison** — the outline's against the rail's — and not `document.elementFromPoint`, which [F05]'s probe showed cannot answer the question at all: the indicator is `pointer-events: none`, so that call returns the rail no matter which of the two is on top. The comparison is sound because the two compare in one stacking context, the workspace wrapper between them being `display: contents`. The flow-click assertions in the same test — one slot's reveal, held under the parked hand — are untouched.
...
- **[F05] — the rail-less edge. Answered: nothing escapes, and no new `overflow` rule is wanted.** Drags on a debug build, sampling the outline every 8ms while a content card crossed a flow strip with a right rail. Its box reached `left: -43` — off the canvas on the rail-less side — and crossed the rail's near edge by 309px on the other, carrying the tile's whole width on every one of 248 samples and never a fraction. Neither overhang escapes the deck: the deck root (`deck-canvas.tsx`, the wrapper holding `containerRef`) already carries `overflow: clip` under the deck's own "the page is not a scroller" law and spans the whole window, so the outline is clipped at the window exactly as a parked pane's frame is. Inside the window the occlusion is the stacking one [B02] asks for, and it composes: the shown workspace wrapper is `display: contents` (`space-layer.css`), so it generates no box and forms no stacking context, and the outline at 8950 compares directly against the margin cap at 8989 and the rail members at 8992 in the canvas container's own context. No `overflow` is added anywhere.

  **And the method that read it is not `elementFromPoint`.** The indicator is `pointer-events: none`, so it is never what `document.elementFromPoint` returns — the call answers what the *pointer* would hit and resolves to the rail whether the outline is ranked above it or below. A probe that ranked the outline at 99960, above the whole band, still got the rail member's editor back from that point. So [B03]'s primary reading cannot see what it was meant to see, and the fallback it names — comparing the two elements' computed `zIndex` — is the reading that actually holds.
