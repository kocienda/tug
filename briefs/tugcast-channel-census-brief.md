...
**[B04] The router grows a per-feed frame/byte counter.** Every claim in [F07] is an estimate because tugcast keeps no per-feed counters; this brief's census question should be answerable by measurement next time. Under [B02] the counter is also the knob's gauge — it shows what each subscription decision actually saves. Counting happens at the router's single fan-out point, in memory, read on demand; it is not a polling loop, not persisted, and never gates a frame. The devise round picks the read-out surface (a `CONTROL` query returning the table is the natural fit, mirroring the existing request/response pairs).

**[B05] Retirement means reserved, never reused.** The stats removal follows the `GIT` precedent: the constants stay in both `protocol.rs` and `protocol.ts` with a doc comment forbidding reuse of the byte. An open `u8` namespace makes silent reuse a wire-compatibility hazard against old clients and recorded captures.
...
- **What is the counter's read-out surface?** [B04] commits to the counter; the devise round picks how it is read. A `CONTROL` query/response is the natural fit, but decide whether the table is per-connection, process-global, or both — per-connection is what shows a subscription's savings, process-global is what answers the census.
...
Phase 2 (subscription protocol + counter): the control-plane message shape, the router honoring per-connection subscription sets with subscribe-everything as the unspoken default, the deck computing the union of card interest, and the per-feed counter [B04] at the same fan-out point — added first within the phase, so the subscription knob's effect is measurable from its first use. Ships with zero behavior change; the tuning — opting real feeds out per card — follows as its own small steps once the knob exists. The open questions above (control-plane home, snapshot replay on late subscribe, where the union is computed, the counter's read-out surface) are what the devise round settles first.

The remaining ride-along candidate (the feed-table drift test) is the devise round's to accept or decline.
