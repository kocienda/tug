...
> **Correction, 2026-09-21 (network-resilience arc).** The finding stands; the deck-side half of the picture it assumes did not. `turn_cancelled` was absent from `KNOWN_CODE_OUTPUT_TYPES`, so the deck dropped every cancel at the ingest guard — see the correction under [B03].
...
> **Correction, 2026-09-21 (network-resilience arc).** "which the deck already renders without a banner" was not true when this was written. The deck had no handler for `turn_cancelled` and no entry for the type in `KNOWN_CODE_OUTPUT_TYPES`, so `frameToEvent`'s guard dropped the frame with no dispatch and no log. The banner-avoidance the decision rests on held for the accidental reason that nothing consumed the frame at all — and the same silence left an escalated Stop stranded, since `turn_complete` was the only clearer of `interruptInFlight`. The deck now commits the cancel quietly through `handleTurnCancelled`, so the decision holds for the reason it always claimed.
