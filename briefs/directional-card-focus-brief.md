...
**[B11] ~~The Session card contributes its transcript verbs to `paneTitleBarItemsStore`.~~ WITHDRAWN 2026-09-11.** It was built as four rows in a `⋮` popup in the pane's rollup (later five, Minimize leading), each rendering its chord for free through `commandShortcut` [F01], and it is now removed root and branch: the card publishes nothing, `session-transcript-title-bar-items.ts` and `at0546` are deleted, and the `⋮` overflow and the `"menu"` presentation are gone from `tug-pane.tsx` and `PaneTitleBarItem` — the title bar has one shape for a card's verb, a standing glyph button.

**Why it failed at the thing it was for.** The decision was sound about the *cost* — [B03] does take a hand's muscle memory and something should pay it back — and wrong about the instrument. A popup menu behind a `⋮`, inside a row that is itself revealed only on hover, shows a reader nothing they were not already going looking for; discoverability is a property of what is *visible at rest*, and nothing about this was. What it did do at rest was put an unexplained glyph on every Session card's masthead, which is how it was found: *"Don't know where this Assorted commands item came from in the session masthead. Don't want this."* The discoverability half of the ask is therefore **still open** — answered today only by the native Session ▸ Go in Transcript submenu and the Keyboard pane, which is where it stood before [B11] and is the honest state. Whatever answers it next must be legible without a gesture, which in the title bar means a glyph and nowhere else means a menu the reader is already reading.
...
5. ~~**The Session card's title-bar rows** [B11] — independent of everything above and landable on its own.~~ Landed, then withdrawn; see [B11]. Its separability held, which is why taking it back out touched nothing in 1–4.
