/**
 * at0485-dash-binding-relaunch.test.ts — a card rotated into a dash stage
 * comes back BOUND after a relaunch ([AT0485]).
 *
 * ## What this gates
 *
 * A dash binding is written against the session id that is the card's at the
 ...
 *
 *   1. a real dash in a scratch repository, so the restore's live-branch
 ...
 *      read for the `^<dash>` sigil — the identity surface that reads the
...
const DASH = "at0485-arc";

const CHECKOUT = realpathSync(resolve(import.meta.dir, "..", ".."));
const INSTANCE_ID = `${process.env.TUG_APPTEST_ID_PREFIX ?? "apptest"}-dash-relaunch-${randomUUID()}`;

const MASTHEAD_DASH = '[data-slot="session-masthead"] [data-slot="session-identity-dash"]';
...
beforeAll(() => {
  ...
  scratch = makeDashScratchRepo({ prefix: "at0485", checkout: CHECKOUT });
  dashId = createDash(projectDir(), DASH, "at0485 relaunch binding", scratch.cli).id;
...
describe.skipIf(!SHOULD_RUN)("at0485 — a dash binding survives a relaunch", () => {
  test(
    "a card resumed on its rotated segment reads bound to the dash it was on",
    async () => {
      ...
      try {
        ...
        // at startup, so a row seeded before one would arrive closed.
        {
          const app = await launchTugApp({
            testName: "at0485-dash-binding-relaunch-A",
        ...
        {
          const app = await launchTugApp({
            testName: "at0485-dash-binding-relaunch-B",
            instanceId: INSTANCE_ID,
            env,
          });
          try {
            ...
            note(`at0485 masthead dash sigil after relaunch: ${JSON.stringify(sigil)}`);
            expect(sigil, "the binding followed the seat").toContain(DASH);
          } finally {
            await app.close();
          }
        }
