 ...
 *      ledger holds, in the order they were laid.
 ...
 *      cooldown, no post policy anywhere in the card.
 *
 * The trip log is asserted as *empty and present*: a tripwire that has never
 * fired has a log, and the card must show the log rather than nothing.
 ...
 *  the rest on hover — printed whole, this is the shape that crushed the rail. */
const LONG_BRIEF =
...
describe.skipIf(!SHOULD_RUN)(
  ...
  () => {
    test(
      "the card lists the laid tripwires, says which are paused, and opens a trip log",
      async () => {
        ...
        try {
          ...
          try {
            ...
            // from the whole card — not merely from the row that used to
            // carry them.
