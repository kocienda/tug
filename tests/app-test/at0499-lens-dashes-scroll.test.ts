/**
 * at0499-lens-dashes-scroll.test.ts — the Lens's Dashes band hands its
 * overflow to its own list's scroller, exactly as the Cards band does.
 *
 * A Lens section is content-sized until the rail runs out of room, and then
 * every band shrinks to its share and scrolls its own body. The Dashes section
 * did not take part: its inner shell carried an automatic block min-content,
 * and a scroll container does NOT shrink an ancestor's min-content — so the
 * band stood at the full intrinsic height of its rows however many there were,
 * and a checkout with more dashes than the rail can show simply ran the last
 * ones off the bottom with no way to reach them.
 *
 * The fixture makes that real rather than arguing it: sixteen dashes, each a
 * document-only dash (a directory under `.tug/dashes/` holding a brief), which
 * is the cheapest real dash there is — no branch, no worktree, and the section
 * renders every one as a row. Sixteen two-line rows are taller than any rail
 * this harness opens, so the section genuinely overflows.
 *
 ...
 *   - the band does not spill — the list's bottom edge lands inside the Lens,
 *     not past it;
 ...
 * @covers tugdeck/src/components/lens/lens-section-band.css
 */
...
const BODY = `${SECTION} .lens-section-body`;
const LIST = `${SECTION} .tug-list-view.lens-dashes-list`;
const ROWS = `${SECTION} [data-slot="lens-document-dash-row"]`;
const LAST_ROW = `${SECTION} [data-slot="lens-document-dash-row"][data-dash="${LAST_DASH}"]`;
...
describe.skipIf(!SHOULD_RUN)("AT0499: the Dashes band scrolls its own rows", () => {
  test(
    "an overflowing section keeps its bottom row reachable",
    async () => {
      const tugbankPath = mkTempTugbank();
      seedTugbankForLaunch(tugbankPath, { sourceTreePath: CHECKOUT });
      const app = await launchTugApp({
        testName: "at0499-lens-dashes-scroll",
      ...
      try {
        ...
        const geometry = await app.evalJS<{
          ...
          `(() => {
             ...
             const lens = document.querySelector(".lens-content");
             return {
        ...
        note("at0499 band geometry", JSON.stringify(geometry));

        // The precondition, asserted rather than assumed: sixteen rows really
        // are more than the band can show. If this ever fails the rail grew,
        ...
        // The list is the scroller, and the band is not: the overflow belongs
        ...
        expect(geometry.bodyScrollHeight).toBeLessThanOrEqual(
          geometry.bodyClientHeight + 1,
        );
        // And the band stays inside the rail instead of running past its foot.
        expect(geometry.listBottom).toBeLessThanOrEqual(geometry.lensBottom + 1);

        note("at0499 lens, dashes band overflowing", (await app.screenshot()).path);
