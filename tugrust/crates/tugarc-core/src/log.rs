...
/// `<repo>/.tug/arcs/<name>/plan.md`, outside every tree git watches — so a
...
mod tests {
    ...
    #[test]
    fn test_validate_arc_name_valid() {
        assert!(validate_arc_name("ab").is_ok());
        assert!(validate_arc_name("login-page").is_ok());
        assert!(validate_arc_name("fix-bug").is_ok());
        assert!(validate_arc_name("test-123").is_ok());
        assert!(validate_arc_name("a1").is_ok());
    }

    #[test]
    fn test_validate_arc_name_invalid() {
        // Too short
        assert!(validate_arc_name("a").is_err());

        // Reserved words
        assert!(validate_arc_name("discard").is_err());
        assert!(validate_arc_name("release").is_err());
        assert!(validate_arc_name("join").is_err());
        assert!(validate_arc_name("status").is_err());

        // Uppercase
        assert!(validate_arc_name("Login-Page").is_err());

        // Special chars
        assert!(validate_arc_name("login_page").is_err());
        assert!(validate_arc_name("login.page").is_err());

        // Leading hyphen
        assert!(validate_arc_name("-login").is_err());
