...
//!
//! The same verb as `tugutil file edit`, through the same function: an edit
//! program in a heredoc reads as one thing, and `tugedit <<'EDIT'` is that
//! thing. The `--patch` mode stays on `tugutil file edit`, which owns the
