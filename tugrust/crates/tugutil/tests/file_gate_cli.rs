//! The `tugutil file gate` decision as the PreToolUse hook actually gets it:
...
fn gate(root: &Path, command: &str) -> Output {
    Command::new(env!("CARGO_BIN_EXE_tugutil"))
        .args(["file", "gate", "--command", command, "--base-dir"])
        .arg(root)
        .output()
        .expect("run tugutil file gate")
}
