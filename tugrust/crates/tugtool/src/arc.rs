...
pub fn dispatch(cmd: ArcCommands, json: bool, quiet: bool) -> ExitCode {
    ...
    let result: Result<(), String> = match cmd {
        ...
        ArcCommands::Commit { name, message } => run_commit(&name, &message, json, quiet),
        ArcCommands::Join {
            name,
            message,
            strategy,
        ...
        ArcCommands::Replay { name } => return run_replay(&name, json, quiet),
        ArcCommands::ResolveBase { name } => return run_resolve_base(&name, json, quiet),
        ArcCommands::Verify { name, base, head } => {
            return run_verify(&name, base, head, json, quiet);
        }
        ArcCommands::Undo { name, list } => {
            return run_undo(name.as_deref(), list, json, quiet);
        }
        ArcCommands::Redo { name, list } => {
        ...
        ArcCommands::Status { name } => run_status(&name, json, quiet),
        ArcCommands::Doctor { name, repair } => run_doctor(&name, repair, json, quiet),
        ArcCommands::Step { name, action } => run_step(&name, action, json, quiet),
        ...
        ArcCommands::Unbind { project, session } => {
...
/// `tugtool arc join --resolve`: run the resolution ladder, then land the candidate
    ...
    let data = match action {
        StepAction::Start { step, through } => {
            let through = through.ok_or_else(|| {
                "arc step start requires --through <m>: the final step of this run's selection \
...
fn print_verify(report: &tugarc_core::surfaces::VerifyReport) {
...
fn print_oplog(ops: &[tugarc_core::OpPayload]) {
    if ops.is_empty() {
...
fn print_redo(outcome: &tugarc_core::RedoOutcome) {
    println!(
...
fn print_undo(outcome: &tugarc_core::UndoOutcome) {
    println!(
...
fn run_arc_run(
    ...
    let tugarc_core::OpenOutcome {
        ...
        resumed,
        record: arc,
    } = tugarc_core::ops::open_arc(&root, name)?;
...
fn post_arc_api(body: serde_json::Value) -> Result<serde_json::Value, String> {
    post_instance_api("/api/arc", "arc binding", body)
}
...
pub(crate) fn post_instance_api(
    ...
    // bind displaced by a running arc reported `unknown_session` instead
    // of naming the arc — on any machine with a second instance live,
...
///
/// `/api/arc` re-resolves at its own door and names the segment it wrote
...
/// records `arc-stop <stage> stopped by user`. `tugtool arc run <name>`
...
fn run_arc_ask(
    ...
    let stopped = answered_session(&response, &session);
    if json {
        print_ok("arc ask", response);
    } else if !quiet {
        println!("Stopped the arc on '{name}' in {stage} to ask (session {stopped})");
        println!("  {question}");
        print_rotation_note(&session, &stopped);
        println!("Resume with tugtool arc run {name} once it is answered");
    }
    Ok(())
}
...
fn claim_arc(name: &str) -> Claim {
    ...
    let resolved = match crate::session_identity::resolve("arc binding", None) {
...
fn broadcast_arc_gone(project: &std::path::Path, arc_id: &str, reason: ArcGone) {
    ...
    for instance in live {
        let url = format!("http://127.0.0.1:{}/api/arc", instance.tugcast_port);
...
mod tests {
    ...
    fn opening_an_arc_writes_one_start_line() {
        ...
        let opened = tugarc_core::ops::open_arc(fixture.root(), "demo").expect("opened");
    ...
    fn opening_the_same_arc_twice_is_one_arc() {
        let fixture = arc_fixture();
        fixture.write_brief("demo");
        tugarc_core::ops::open_arc(fixture.root(), "demo").expect("opened");
        let reopened = tugarc_core::ops::open_arc(fixture.root(), "demo").expect("reopened");
        assert!(
            !reopened.started,
    ...
    fn resuming_a_stopped_arc_writes_the_resume_and_clears_the_stop() {
        ...
        let opened = tugarc_core::ops::open_arc(fixture.root(), "demo").expect("resumed");
        assert!(!opened.started);
        assert!(opened.resumed);
        ...
        let again = tugarc_core::ops::open_arc(fixture.root(), "demo").expect("still open");
    ...
    fn an_arc_stopped_by_a_closing_card_resumes_from_a_fresh_one() {
        ...
        let opened = tugarc_core::ops::open_arc(root, "demo").expect("resume");
