...
Each directory here is one real edit the model made to a repo file from Bash, lifted from a Claude Code session transcript for this checkout, paired with the edit program that expresses it.
...
- `program.edit` — the edit program. `patch_block_replace` is the triple-quoted `(old, new)` pair expressed as a `patch` hunk, where the prefix column carries the file's own indentation instead of the writer reconstructing it.
...
`edit_corpus.rs` runs the command in one temp dir and `tugedit` in another and asserts the results are byte-identical. The oracle is the real interpreter, so a program that silently diverges from what the command actually did fails here.
