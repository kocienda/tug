...
fn step_in(
    ...
    if base_plan_dirt(repo_root, &rel).is_dirt() {
        return Err(format!(
            "base copy of the plan has uncommitted changes at {rel}; run: tugtool dash adopt-plan {name}"
        ));
    }
...
fn replay_ledger_progress(base_body: &str, worktree_body: &str) -> (String, Vec<String>) {
    let Ok(worktree_doc) = tugtool_core::plan::parse(worktree_body) else {
    ...
    let Ok(base_doc) = tugtool_core::plan::parse(base_body) else {
    ...
    for row in progressed {
        ...
        match incoming {
            ...
            Some(_) => {
                match tugtool_core::plan::set_ledger_status(
                    &body,
                    &row.anchor,
                    &row.status,
                    row.commit.as_deref(),
    ...
    let incoming: Option<String> = if !work_present {
        ...
        if base_body == work_body {
            ...
            let same_content = match (
                tugtool_core::plan::parse(&base_body),
                tugtool_core::plan::parse(&work_body),
            ) {
                (Ok(base_doc), Ok(work_doc)) => {
                    tugtool_core::plan::content_stamp(&base_doc, &base_body)
                        == tugtool_core::plan::content_stamp(&work_doc, &work_body)
                }
                _ => false,
            };
            if same_content {
...
fn warn_on_superseded_worktree_edits(
    ...
    let head_doc = tugtool_core::plan::parse(&head_body).ok()?;
    let work_doc = tugtool_core::plan::parse(work_body).ok()?;
    let changed = tugtool_core::plan::content_stamp(&head_doc, &head_body)
        != tugtool_core::plan::content_stamp(&work_doc, work_body);
    changed.then(|| {
...
/// `sessions.db`, or no row for that id. `tugtool dash commit` runs inside a
...
fn plan_remedy_sentence(paths: &[String], plan: Option<&str>, name: &str) -> Option<String> {
    ...
    paths.iter().any(|p| p == rel).then(|| {
        format!(" This includes the dash's own plan ({rel}) — run: tugtool dash adopt-plan {name}.")
    })
}
...
"#;
    ...
    fn stamp_of(body: &str) -> String {
        let doc = tugtool_core::plan::parse(body).unwrap();
        tugtool_core::plan::content_stamp(&doc, body)
    }
    ...
    fn a_plan_adopted_at_birth_survives_the_whole_run_and_lands() {
        ...
        let doc = tugtool_core::plan::parse(&landed).unwrap();
        assert!(
