...
fn park_a_leased_conflict(root: &Path, name: &str) -> String {
    ...
    let outcome = tugarc_core::resolve::resolve_conflicts(root, name, None).unwrap();
    assert!(
        !outcome.unresolved.is_empty(),
        "the fixture must actually conflict"
    );
    tugarc_core::resolve::mark_resolve_begun(root, name).expect("the begin marker lands")
}

fn conflict_tip(root: &Path, name: &str) -> Option<String> {
    tugarc_core::resolve::read_conflict(root, name).map(|c| c.tip)
}
...
fn arc_join_names_a_live_resolve_and_break_lease_lands_it() {
    ...
    preview.args(["arc", "join", "demo", "--preview", "--json"]);
    ...
    broke.args(["arc", "join", "demo", "--break-lease", "--json"]);
...
fn arc_join_resolve_refuses_over_a_live_chain_and_leaves_it_standing() {
    ...
    assert!(
        tugarc_core::resolve::read_candidate(&root, "demo").is_none(),
...
fn arc_bind_and_unbind_post_to_the_instance_and_emit_envelopes() {
    ...
    assert_eq!(envelope["command"], "arc bind");
...
/// needed: `/api/arc` resolves at its own door, so the bind still lands on
...
fn arc_redo_reverses_an_undo_and_then_says_there_is_nothing_left() {
    ...
    assert!(run(&["arc", "join", "cli"]).status.success());
    let landed = git_stdout(&root, &["rev-parse", "main"]);
    assert!(run(&["arc", "undo"]).status.success());
    assert_ne!(git_stdout(&root, &["rev-parse", "main"]), landed);

    let redo = run(&["arc", "redo"]);
    assert!(
        redo.status.success(),
        "redo failed: {}",
    ...
    let via_undo = run(&["arc", "undo", "--list"]);
    let via_redo = run(&["arc", "redo", "--list"]);
...
/// than a crash-hook test: nothing in `tugarc-core` knows it is being watched,
...
/// Run `tugtool arc join <name>` under the shim and SIGKILL it the moment the
...
fn continue_join(tmp: &Path, root: &Path, name: &str) {
    let mut cmd = tug(tmp);
    cmd.current_dir(root);
    cmd.args(["arc", "join", name, "--continue"]);
    let out = cmd.output().unwrap();
    assert!(
        out.status.success(),
        "--continue failed: {}",
...
fn arc_bind_dry_run_names_the_segment_and_writes_nothing() {
    ...
    dry.args(["--json", "arc", "bind", "demo", "--dry-run"]);
...
fn arc_bind_dry_run_says_it_wrote_nothing() {
    ...
    dry.current_dir(&root);
    dry.env("TUG_SESSION_ID", "seg-old");
    dry.args(["arc", "bind", "demo", "--dry-run"]);
    let out = dry.output().unwrap();
    assert!(out.status.success());
...
fn a_refused_claim_fails_arc_create_and_says_so_in_the_json() {
    ...
    create.args(["--json", "arc", "create", "demo"]);
