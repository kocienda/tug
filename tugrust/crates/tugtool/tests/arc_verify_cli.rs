//! `tugtool arc verify` end to end, against a synthesized project that is not
//! Tug.
//!
...
fn verify(tmp: &Path, root: &Path, args: &[&str]) -> Output {
    tug(tmp)
        .args(["arc", "verify", "demo"])
        .args(args)
        .current_dir(root)
        .output()
        .expect("failed to run tugtool arc verify")
}
...
fn json_carries_the_same_verdict_counts_and_receipt_as_the_text() {
    ...
    let json: serde_json::Value =
        serde_json::from_str(&stdout_of(&verify(tmp.path(), &root, &["--json"])))
            .expect("valid JSON");
    assert_eq!(json["command"], "arc verify");
