//! Integration tests for `tugtool file edit` and the `tugedit` binary, driving
...
#[test]
fn tugedit_and_tugtool_file_edit_are_the_same_verb() {
    let (_dir_one, one) = init_repo();
    let (_dir_two, two) = init_repo();
    let program = "file a.txt\n  replace 'two' with 'deux'\n  append <<\nfour\n>>\n";

    let through_tugtool = edit(&one, &["--preview"], program);
    let through_tugedit = tugedit(&two, &["--preview"], program);
    assert_eq!(code(&through_tugtool), 0);
    assert_eq!(code(&through_tugedit), 0);
    assert_eq!(through_tugtool.stdout, through_tugedit.stdout);
