...
//! something that is not a deck — a background spawner rather than a card —
//! still needs an owner in that field, so it carries `background:<owner>`
//! instead: a string that names who asked rather than addressing anything.
//! This module is where that convention is minted and where it is recognised,
//! so the next background spawner inherits the one answer to "does this
//! `card_id` name a deck card?" rather than inventing a second prefix ([B04]).
//!
...
pub(crate) fn is_background_card_id(card_id: Option<&str>) -> bool {
    card_id.is_some_and(|id| id.starts_with(BACKGROUND_CARD_PREFIX))
}
...
mod tests {
    ...
    fn a_minted_id_is_recognised() {
        ...
        assert_eq!(id, "background:nightly-audit");
    ...
    fn unbound_and_deck_cards_are_not_background() {
        ...
        // exactly right.
        assert!(!is_background_card_id(Some(
            "card-background:nightly-audit"
        )));
    }
}
