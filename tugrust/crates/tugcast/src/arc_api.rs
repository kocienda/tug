...
pub(crate) fn bind(
    ...
    if crate::wheel::arc_is_running(ledger, tug_session_id)
    ...
    let dash_id = match tugarc_core::ops::ensure_dash_id(project_dir, dash) {
        Ok(id) => id,
...
/// record` says it, and the Dashes card says it. The only surface missing is
...
pub(crate) fn arc_stop(
    ...
    let Some(record) = tugarc_core::arc::read_arc(project_dir, dash) else {
        return ArcApiOutcome::Error(format!("{dash} has no arc to stop"));
    };
    if record.done {
        return ArcApiOutcome::Error(format!("{dash}'s arc is already done"));
    }
...
fn seated_stages(ledger: &SessionLedger, dash_id: &str) -> Vec<SeatedArcStage> {
    ...
    for session_id in sessions {
        ...
        let Some(record) =
            tugarc_core::arc::read_arc(std::path::Path::new(&row.project_dir), &dash_name)
        else {
            continue;
        };
        if record.done || record.stopped.is_some() {
...
mod tests {
    ...
    fn on_arc_card(root: &std::path::Path) -> SessionLedger {
        ...
        ledger
            ...
            .unwrap();
        tugarc_core::arc::append_arc_start(root, "alpha", "dash/alpha-brief.md").unwrap();
        ledger
    }
    ...
    fn a_card_running_a_score_refuses_a_bind_to_another_dash() {
        ...
        match outcome {
            ...
            _ => panic!("a live arc must refuse a bind naming another dash"),
        }
        assert_eq!(
    ...
    fn dash_gone_finds_the_sessions_a_stage_is_seated_on() {
        ...
        let ArcApiOutcome::Cleared { cleared, seated } =
    ...
    fn an_ending_writes_no_arc_line_after_the_terminal_one() {
        ...
        tugarc_core::log::append_dash_log(root, "alpha", "discarded", "").unwrap();
    ...
    fn arc_stop_names_the_stage_it_will_stop() {
        ...
        match arc_stop(&ledger, root, "claude-1", "alpha") {
            ArcApiOutcome::ArcStopped { dash, stage, .. } => {
