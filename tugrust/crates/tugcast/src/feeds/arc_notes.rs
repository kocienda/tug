...
//!
//! The first cut had each `tugtool arc` verb post its own announcement. That
...
fn is_news(line: &str, floor: &str) -> bool {
    tugarc_core::log::split_log_line(line).is_some_and(|(timestamp, _, _, _)| timestamp >= floor)
}
...
/// filtered every line of the wheel machinery's first fully-instrumented run
...
fn closed_step(marker: &str, note: &str) -> Option<u32> {
    matches!(marker, "step-done" | "step-withdrawn")
        .then(|| tugarc_core::log::read_step_fields(note).map(|(current, _)| current))
        .flatten()
}
...
pub fn note_for_line(dash: &str, marker: &str, note: &str) -> Option<String> {
    ...
    match marker {
        ...
        "step-start" | "step-done" | "step-withdrawn" | "step-reset" | "step-reopen" => {
            ...
            let tail = tugarc_core::log::read_step_title(note, current);
