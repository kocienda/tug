...
//!
//! A tripwire with no probe needs no worktree, so its trip never spawns a session:
//! it asks a pooled worker one question and reads the answer. That is the
//! cheap path, and it is what makes a standing tripwire affordable enough to leave
//! armed.
//!
...
//! so a machine whose tripwires never fire pays nothing at all.
//!
...
//! put words in the channel nobody wrote. A tripwire's is the opposite: it was
//! asked a question and the trip log has to say it did not answer, or a tripwire
//! that has been broken for a week looks exactly like a tripwire with nothing to
//! report.
...
/// The one job a tripwire pool runs.
pub const TRIPWIRE_VERDICT: &str = "tripwire-verdict";

/// Ceiling on a verdict turn. Past it the trip fails rather than waiting: a
/// tripwire is background work, and a background question nobody is watching must
/// not hold a concurrency slot indefinitely.
const TRIPWIRE_VERDICT_TIMEOUT: Duration = Duration::from_secs(120);

/// Turnaround past which the call is marked slow. Nothing is cancelled — it
/// only makes drift readable in accumulated logs.
const TRIPWIRE_VERDICT_SLOW: Duration = Duration::from_secs(45);

/// The model a tripwire with no `model` column runs on.
pub const DEFAULT_TRIPWIRE_MODEL: &str = crate::feeds::overview_agent::DEFAULT_MODEL;

/// The verdict tier's whole contract.
///
/// Generic on purpose: every tripwire shares this job, and what makes one tripwire
/// different from another is its brief and its evidence, which arrive as the
/// turn's input. A per-tripwire job class would be a pool per tripwire, which is a
/// process per tripwire, for a question that takes one turn.
///
/// The two fields the wording works hardest on are `interest` and `outcome`.
/// `interest` is the only thing standing between a tripwire and a channel full of
...
/// could not have done.
const TRIPWIRE_VERDICT_INSTRUCTIONS: &str = "\
TRIPWIRE VERDICT

A standing tripwire fired. You are given the tripwire's brief — what its author \
...
`routine` otherwise. Most firings are routine. A tripwire fires on a pattern, and \
...
const TRIPWIRE_AGENT_JOBS: &[JobSpec] = &[JobSpec {
    name: TRIPWIRE_VERDICT,
    timeout: TRIPWIRE_VERDICT_TIMEOUT,
    slow: Some(TRIPWIRE_VERDICT_SLOW),
    instructions: TRIPWIRE_VERDICT_INSTRUCTIONS,
}];

/// Build a pool pinned to one model. Spawns nothing — the first firing is what
/// spawns the worker, so an armed tripwire that never trips costs nothing.
///
/// One worker, because a tripwire's turn is not on anybody's critical path: a
/// second concurrent firing waiting a turn is cheaper than a second `claude`
/// process resident for the life of the instance.
pub fn build_tripwire_pool(
    model: &str,
...
///
/// Lazy: a model named by no tripwire never gets a pool, and a pool that exists
/// has still spawned nothing until a tripwire on that model actually fires. The
/// map only grows, which is correct for a process whose tripwire set is small and
/// whose models are drawn from a handful of names.
pub struct TripwirePools {
    spawner: Arc<dyn AgentWorkerSpawner>,
...
impl TripwirePools {
    ...
    /// The pool for a tripwire's model, building it on first ask. A tripwire with no
    /// model column runs on the default rather than on nothing.
    pub fn for_model(&self, model: Option<&str>) -> Arc<SharedAgentPool> {
        let model = model
            .map(str::trim)
            .filter(|m| !m.is_empty())
            .unwrap_or(DEFAULT_TRIPWIRE_MODEL);
        let mut pools = self.pools.lock().expect("tripwire pools mutex");
        Arc::clone(
            pools
                .entry(model.to_string())
                .or_insert_with(|| build_tripwire_pool(model, Arc::clone(&self.spawner))),
        )
    }
    ...
    pub fn pool_count(&self) -> usize {
        self.pools.lock().expect("tripwire pools mutex").len()
    }
}
...
/// The ref kinds a tripwire may name. A kind outside this set is dropped rather
...
/// What a tripwire's turn answered.
#[derive(Debug, Clone, PartialEq, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct TripwireEnvelope {
    pub interest: Interest,
    pub outcome: Outcome,
    pub headline: String,
    #[serde(default)]
    pub refs: Vec<TripwireRef>,
}

impl TripwireEnvelope {
    /// The refs worth keeping — every one whose kind this build can resolve,
    /// in the order the model wrote them.
    pub fn known_refs(&self) -> Vec<TripwireRef> {
        self.refs
            .iter()
...
fn parse_one(span: &str) -> Option<TripwireEnvelope> {
    if let Ok(envelope) = serde_json::from_str::<TripwireEnvelope>(span) {
        return Some(envelope);
    }
    let value: serde_json::Value = serde_json::from_str(span).ok()?;
    let inner = value.as_object()?.get("verdict")?;
    serde_json::from_value::<TripwireEnvelope>(inner.clone()).ok()
}

/// The turn's input: what the tripwire wants to know, then what happened.
///
...
/// the report, the counts — and the tripwire's entire value is that a diagnosing
...
mod tests {
    ...
    fn envelope(raw: &str) -> TripwireEnvelope {
        parse_tripwire_envelope(raw).expect("an envelope")
    }
    ...
    fn an_unreadable_answer_is_no_envelope_at_all() {
        assert!(parse_tripwire_envelope("I could not tell.").is_none());
        assert!(parse_tripwire_envelope("{\"interest\":\"routine\"").is_none());
        assert!(
            parse_tripwire_envelope(r#"{"interest":"routine","outcome":"verdict"}"#).is_none(),
            "a missing headline is a missing field, not a blank one"
        );
        assert!(
            parse_tripwire_envelope(r#"{"interest":"maybe","outcome":"verdict","headline":"h"}"#)
                .is_none(),
            "an interest outside the two spellings is not read as either"
        );
        assert!(
            parse_tripwire_envelope(
    ...
    fn an_unknown_ref_kind_is_dropped_and_the_rest_are_kept() {
        let e = envelope(
            r#"{"interest":"routine","outcome":"verdict","headline":"h","refs":[{"kind":"file","target":"a.rs"},{"kind":"portent","target":"raven"},{"kind":"dash","target":"tripwire-w-abc"}]}"#,
        );
    ...
    fn a_pool_is_built_once_per_model_and_a_null_model_takes_the_default() {
        ...
        let pools = TripwirePools::new(spawner);
    ...
    async fn a_verdict_turn_carries_the_job_instructions_and_the_composed_input() {
        ...
        let answer = pool
            .run(
                TRIPWIRE_VERDICT,
                compose_input("watch the edits", "the marker"),
            )
            .await
            .expect("the pool answers");
        assert_eq!(
            parse_tripwire_envelope(&answer).unwrap().headline,
            "nothing to see"
        );

        let seen = spawner.turns_seen();
        assert_eq!(seen.len(), 1);
        assert!(seen[0].starts_with("TRIPWIRE VERDICT"), "{}", seen[0]);
