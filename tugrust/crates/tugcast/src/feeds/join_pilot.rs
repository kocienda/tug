...
//!
//! An arc that has finished the work somebody asked it for wants machine work
...
//! inverted — the person came to land a finished arc and was handed a chore.
//!
...
//!
//! The trigger is `ArcDetail.join_ready` — a fact `tugarc_core` derives from
//! the arc's own recorded telemetry: a declared step selection reaching its
...
//! the unblock for arcs older than the derivation — but nothing gates on it.
//!
//! This module is the predicate that decides whether an arc wants that work.
...
//! derived readiness bool, whether any live session is bound to the arc, and
//! the `ArcJoinState` the board just computed. That is deliberate: the caller
//! sits on a path that already does a blocking git walk, once per arc.
//! `ArcDetail` carries no head sha, and
...
//! two new git subprocesses per arc per recompute, on exactly the arcs the
//! pilot most wants to act on.
//!
//! # The attempt mark is not the predicate's business
//!
//! The mark that stops a re-kick (`branch.tugarc/<name>.tugjoinpilot`, a
//! `<base_sha>:<arc_head>` pair) is read and written by the dispatch, under the
...
//!
//! Either head moving is new work — a round on the arc, or a base the arc
//! has not been reconciled against — so the pair is exactly the fact that says
//! "this reconcile has not been attempted". Keyed on the arc head alone the
//! pilot would never re-run after the base moved, and an arc cut days ago
...
use tugcast_core::types::ArcJoinState;

use super::join_occupancy::{self, JoinOccupancy, JoinRunKind};

/// What the pilot should do about an arc, when it should do anything.
...
/// What the pilot should consider doing about this arc, if anything.
///
...
///    the arc, so a reconcile on an unbound arc is spent on nobody. Binding
///    a card re-enters the arc on the next recompute; `/join <name>`
///    reconciles on demand regardless.
/// 3. A run holds the arc — occupancy would refuse anyway.
/// 4. Blockers — a blocked arc needs an act that lives elsewhere.
...
pub fn pilot_action(join_ready: bool, bound: bool, state: &ArcJoinState) -> Option<PilotAction> {
    if !join_ready || !bound {
        return None;
    }
...
pub trait PilotRunner: Send + Sync {
    ...
    /// press would.
    async fn reconcile(&self, project_dir: &str, arc: &str, occupancy: JoinOccupancy);
}
...
/// Dispatch the pilot's work for one arc, off the recompute (Spec S06).
///
...
pub fn dispatch(repo_root: &Path, arc: &str, action: PilotAction) {
    ...
    tokio::spawn(async move {
        run_dispatch(runner.as_ref(), repo_root, arc, action).await;
    });
}
...
/// recomputes racing the same arc produce exactly one run. The head pair is
...
async fn run_dispatch(
    ...
    let owner_key = tugarc_core::ops::arc_owner_key(&repo_root, &arc);
    ...
    let heads = tokio::task::spawn_blocking(move || head_pair(&probe_root, &probe_arc)).await;
    let Ok(Some((head_pair, arc_head))) = heads else {
        tracing::debug!(arc = %arc, "join-pilot: no head pair; skipping");
        return;
    };

    let occupancy = match join_occupancy::acquire(&owner_key, kind, Some(arc_head)) {
        Ok(guard) => guard,
        Err(detail) => {
            // Busy is not an error: the next recompute reconsiders.
            tracing::debug!(arc = %arc, detail = %detail, "join-pilot: arc is busy");
            return;
        }
    };

    let mark_root = repo_root.clone();
    let mark_arc = arc.clone();
    let mark_pair = head_pair.clone();
    let claimed = tokio::task::spawn_blocking(move || {
        if tugarc_core::verify::read_pilot_mark(&mark_root, &mark_arc).as_deref()
            == Some(mark_pair.as_str())
        {
            return false;
        }
        // Written before the run, deliberately (Risk R01).
        let _ = tugarc_core::verify::write_pilot_mark(&mark_root, &mark_arc, &mark_pair);
        true
    })
    .await;
    ...
    match action {
        PilotAction::Reconcile => runner.reconcile(&project_dir, &arc, occupancy).await,
    }
}

/// `(<base_sha>:<arc_head>, arc_head)` for an arc, or `None` when either side
/// will not resolve.
fn head_pair(repo_root: &Path, arc: &str) -> Option<(String, String)> {
    let detail = tugarc_core::ops::arc_detail_entry_in(repo_root, arc)?;
...
mod tests {
    use super::*;
    use tugcast_core::types::{ArcJoinBlocker, ArcJoinQuestion, ArcJoinState};

    /// An arc with nothing standing in the way and no candidate — the state a
    /// freshly built, conflicted arc arrives in.
    fn bare() -> ArcJoinState {
        ArcJoinState {
    ...
    #[test]
    fn only_a_ready_bound_arc_is_piloted() {
        // An arc still being worked wants nothing from the pilot, whatever else
        // is true of it.
    ...
    fn blockers_stop_the_pilot() {
        let mut state = bare();
        state.blockers.push(ArcJoinBlocker {
    ...
    fn a_question_or_a_stuck_line_stops_the_pilot() {
        let mut asked = bare();
        asked.question = Some(ArcJoinQuestion {
    ...
    impl PilotRunner for CountingRunner {
        async fn reconcile(&self, _project_dir: &str, _arc: &str, occupancy: JoinOccupancy) {
            drop(occupancy);
    ...
    /// A repo with one commit on `main` and one `tugarc/<name>` branch — the
    /// shape `arc_detail_entry_in` reads a base and a head off.
    fn repo_with_arc(name: &str) -> (tempfile::TempDir, PathBuf) {
    ...
    async fn the_pilot_runs_once_per_head_pair() {
        let (_dir, root) = repo_with_arc("once");
    ...
    /// An arc somebody else holds is left alone — busy is not an error, and
    /// the next recompute reconsiders.
    #[tokio::test]
    async fn a_busy_arc_is_not_dispatched() {
        let (_dir, root) = repo_with_arc("busy");
        let (runner, reconciles) = counting();

        let owner_key = tugarc_core::ops::arc_owner_key(&root, "busy");
