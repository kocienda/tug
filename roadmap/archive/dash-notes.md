/tugplug:plan-devise The join arc's machinery is done ([D147]/[D148]/[D149], roadmap/join-arc-2.md 6/6) and a real join on 2026-08-22 (apptest-ledger → 0b2d404ca) proved the spine: arming fired on the last step-done, the pilot reconciled silently, the prompt raised itself, the squash landed with the authored draft, bindings swept. What failed was every part of how the arc speaks. Five items, all with captured evidence from that run. (a) Inline the decision. The join prompt mounts as a pane-modal TugSheet (join-prompt-sheet.tsx, showSheet title "Join?" ~line 311; wired session-card.tsx:3474-3507) that scrims and inerts the pane body — greying out the run's ending narration, the exact context the decision needs. Retire the sheet mount and render the same QuestionWizard (session-question-dialog.tsx:1105) inline the way AskUserQuestionToolBlock mounts it. Everything fact-driven stays untouched: durable DashJoinPrompt on the changesets feed, request_id staleness refusal, head-keyed dismissal mark, answer via changeset_join_prompt_answer. Amend [D142]'s "card-hosted modal" wording — presentation detail, not machinery. (b) An honest landing phase. After "Join now" the surface must stop asking: no "Join?" header during execution. Progress beats are fully plumbed (changeset_join_land_delta from agent_supervisor.rs:5205-5232; BEAT_WORDS squashing/tearing down/releasing/recording in dash-join-register.ts:88-93) but the real join rendered only the progress===null arm (join-prompt-sheet.tsx:185). Find the key mismatch — the store keys `${workspaceKey}|${dash}`, frames echo request.dash, the reader passes boundDashEntry.display_name (session-card.tsx:3477) — and pin with a test that a prompt-route join renders at least one beat. (c) Terminal state is transcript ink, not composer furniture. "Joined X into main | joined" is DashJoinRegisterView in the composer statusRow (tug-prompt-entry.tsx:3921), driven by JoinModeController.narration, which clears only by replacement and so rests indefinitely. A completed join should land one status row in the transcript (decide its ink class against [D111]) and the composer register should clear; the Lens and shade mounts of dashJoinRegister() stay. (d) The lands-as lie, fixed under [L29]. The prompt announced "landing with the branch description — no draft was written" while the join landed with the authored draft — preview and execution both resolve through landing_message_preview → dash_draft_message (ops.rs:2703) yet disagreed. Draft rows are persisted keyed through the canonicalization gateway (server.rs:342); the read side compares against project_spellings (ops.rs:2664), which is bare canonicalize plus the raw string — an [L29] violation ("every persisted or compared path routes through the canonicalization gateway — never a raw path, never bare canonicalize"). Reproduce the miss first, then fix by routing the read-side key through the same gateway the write side used; do NOT widen project_spellings into a tolerate-more-spellings shim — if crate layering keeps tugdash-core from reaching the gateway, the layering is part of the defect, and the fix may belong at the caller that hands dash_draft_message its repo_root. While in there, rule out the second failure mode: whether tugutil draft set bumps the changesets feed so the standing prompt recomposes its message rather than serving a stale one. A lands-as that can't be trusted is worse than none. (e) Doctrine sweep: tuglaws/dash-work-doctrine.md "The join finishes itself" (~lines 119-175) still describes the verdict gate, --anyway, the durable override, and verify_tier0/verify_tier1 — all deleted by [D149]. Rewrite to the reconcile-clean gate and run-ending verification. 🢂 roadmap/join-voice.md


/tugplug:plan-devise The dash/join workflow must be general-purpose, and it has been drifting toward Tugtool-only. Assumes roadmap/join-voice.md has landed; if it has not, say so and stop. The engine itself is already project-agnostic — tugdash-core knows no Justfile, and the universe seam, dash create --base, and at0441's scratch-repo arc prove the verbs against repos that are not Tugtool — so this plan does NOT touch the verb layer or the join arc. The specificity has pooled in two places, and the second was created deliberately by [D149]. (1) The verification seam. [D149] deleted the verify_tier0/verify_tier1 config keys, which were the only per-project declaration the workflow had for "how do I check this tree"; verification now lives in skill prose plus a repo-local scripts/verify-fit.sh that a brand-new project will not have. Extend the one seam that survived — the [tugtool.dash].post_create hook pattern — with per-project declarations for the run's ending: a verify command (the verify-fit equivalent, invoked scoped to a diff), a build command (what just app-debug is here), and decide whether test selection wants one too or stays the plan's business. Decide where declarations live by reading where post_create lives today and following it. Name the [D149] boundary explicitly in the plan: this seam serves the RUN's ending under [D149]'s own doctrine — verification stays out of the join, and the two forbidden alternatives (terminal sandbox sweep, join-time advisory) stay forbidden; a reviewer must not be able to read this as re-proposing them. Tugtool then becomes the first consumer: its own config declares verify-fit.sh and just app-debug, and the hardcoded references in tuglaws/devise-skeleton.md and the skills route through the declaration. (2) The skills speak only Tugtool. tugplug ships in the app bundle — it is the product's onboarding voice — yet dash-implement, dash-on, plan-devise, plan-review, dash-join, dash-audit, and draft speak fluent Tugtool: just recipes, cargo nextest, bun, app-test discipline, tuglaws paths. Inventory every project-specific assumption in the skill prose, rewrite each against the declaration seam, and give every absence a stated degradation in the pattern plan-review already uses for a missing rubric ("proceed on the criteria above and say so"): no verify command declared → the checkpoint is the plan's own commands, said plainly, never invented; no tuglaws/ → the review names its criteria inline; no build command → no build is offered. Sweep tugplug/hooks the same way — gate-app-test-output.sh and friends must no-op cleanly on a project with no app-tests rather than misfire. Remember the skills and hooks run from the app bundle, not the repo — repo edits do nothing until rebuilt or copied in, so live testing needs the cp step. (3) The proof is a scratch project, per the real-not-fake rule: a repo with none of Tugtool's machinery — no Justfile, no app-tests, no tuglaws — walked end to end through plan-devise → dash-implement → the join prompt → landed, with every degradation sentence observed in the actual transcript rather than asserted. The factory-fresh VM lab is the eventual venue for the full onboarding proof but is out of scope here; a local scratch repo is the bar. (4) Record the seam and the degradation doctrine as a [D##] in tuglaws/design-decisions.md so the next skill author writes against the declaration instead of the dialect. 🢁 roadmap/dash-generality.md



/tugplug:plan-devise Implement the unified Changes pass — the shade design settled 2026-08-16 and recorded in roadmap/archive/dash-closure-brief.md §2, never built. Assumes roadmap/join-voice.md has landed (the join prompt is inline and the composer join register clears on completion); if it has not, say so and stop. Four settled items. (1) Z4A reads Prompt | Changes, always — the conditional Join segment is deleted; LANDING_WORDS keeps Commit/Join as the Z5 land-button labels, so the join act loses no surface, only the mode toggle stops shape-shifting ([D122] is the law this honors). (2) The collapsed Dashes fold dies: non-fronted dashes render as compact rows in the shade's one list discipline ([D143] governs the row anatomy — identity, work, join state standing; evidence folding). (3) Guarded release: the shade may release a dash's worktree when it is this session's own fronted dash, or when the dash's bound_sessions names no session open in this instance; the confirm popover is a fact sheet stating what release does to which files — never "are you sure". Remember dash release hands a dirty worktree's files back to the base checkout, so the facts on that sheet are load-bearing. (4) Scope hygiene at landing: verify strip_dash_scope (tugdash-core/src/ops.rs) covers every landing route so any leading tugdash(…): is stripped rather than doubled, and confirm all three draft-authoring skills (dash-implement, dash-on, dash-join) instruct bare subjects — dash-implement and dash-on already do; check dash-join. Cross-check tuglaws/tuglaws.md and pane-model.md before touching Z4/Z5 chrome and name the laws in the commits. 🢂 roadmap/unified-changes.md


/tugplug:plan-devise Entry points into dash workflows. Today a dash starts from slash-command archaeology: dash create lives in skills and composer routes, bind is implicit in the verbs, and join is a prompt that raises itself — but no surface that *shows* a dash lets you *act* on one. The surfaces that show dashes: the Lens Dashes section rows, the Changes shade's dash lanes, and the session masthead's dash cluster. The acts worth offering: start a dash (create+bind from a plan or plan-less), bind/unbind an existing dash to this session, open the join composer mode on a joinable dash, and trigger dash replay on a deferred or conflicted unbound dash (the closure brief's lane-click-affordance item — fold it in here, it is the same gesture family). This is a design round before it is a plan: settle with AskUserQuestion during devise which acts appear on which surface and in what form (row menu, hover affordance, click-through), whether creation belongs on a display surface at all or stays a composer act, and how the affordances respect "status is never a control" ([D142]) — a row that displays state must not turn into a button by surprise. Honor the boundaries already law: only the user lands, dash join --resolve lands and is never a probe, guarded release rules from the unified Changes pass (roadmap/unified-changes.md — assume it landed; if not, say so and stop). Then plan the settled answers. 🢂 roadmap/dash-entry-points.md


/tugplug:plan-devise Triage and close the dash/join tail — the small items left standing after join-voice, unified-changes, and dash-entry-points landed (assume all four; if not, say so and stop). Candidates, each to be confirmed wanted or cut by name into Non-goals via AskUserQuestion before any step is written: (1) a "joined" archaeology surface for browsing past landed dashes — today a landed dash vanishes from every face the moment broadcast_dash_gone sweeps it; (2) dash bind/unbind --instance/--port selection for multi-instance machines; (3) delete-or-graduate DashFactsRun/dash-facts.tsx, the orphaned facts component join-arc-2 left behind; (4) queue-a-landing-for-turn-end from the archived closure brief's pull-driven tail; (5) teaching the injected conflict turn to run checkpoints after rebase; (6) head+tail elision for capped review diffs. Keep the plan small — this is a closure sweep, not a campaign; anything that grows past a step or two of real design belongs in its own future plan and should be cut here with a pointer. 🢂 roadmap/dash-join-tail.md
