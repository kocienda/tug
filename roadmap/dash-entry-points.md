...
So bind/unbind and discard are solved. What no surface offers: a way *from a dash row* to the room where the join decision lives; any manual trigger for `dash replay` (the closure brief's deferred lane-click item — `roadmap/archive/dash-closure-brief.md` names it *"a lane click affordance for `dash replay` on a deferred/conflicted dash with no bound session"*, though [P03] widens the population past the brief's "no bound session" framing, for reasons the code settles); and a ruling on what a click on a dash row means anywhere ([D142]: status is never a control).
...
- `roadmap/unified-changes.md` landed as `51bb1eaae` ([D152]) — verified in `git log` on 2026-08-22.
