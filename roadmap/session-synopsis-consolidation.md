...
- Every symbol in Table T01's "delete" rows has zero remaining references outside `roadmap/`. (`rg` per symbol)
...
Run from the repo root; **scope excludes `roadmap/`** (plans and archives are history) and `tests/model-eval/__pycache__`:
