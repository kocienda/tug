...
- `roadmap/join-hardening-brief.md` — the findings, their file:line evidence, and the audit's "found sound" list, which this plan builds on and does not re-litigate.
...
- [ ] `roadmap/join-hardening-brief.md`: a one-line status note at top pointing here (the brief stays as the findings record).
