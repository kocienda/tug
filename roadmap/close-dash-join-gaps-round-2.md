...
| Workshop worktree leaks on crash | med | med | Workshop lives under the dash homes with a nonce name; `clear_candidate`/discard sweeps it; `git worktree remove --force` teardown mirrors the Justfile sweep hardening | stray worktrees after a red run |
...
- Workshop paths are repo-universe citizens: created via the same git the dash verbs use, invisible to the changes card (outside any session workspace), and swept on discard/join. The `tugworkshop/<name>` branch is never a join target, and because it lives outside `refs/heads/tugdash/` it cannot appear on any dash surface — the exclusion needs no filter.
...
- **The surface is `QuestionWizard`**, the frameless rail-and-panel question surface (2–4 options, a free-text reply path, focus already managed). `TugInlineDialog` is the primitive `QuestionWizard` is built from; composing that instead would be hand-rolling a component that already exists. What the face must **not** reuse is the session machinery around it — `pendingQuestion`, `handleRespondQuestion`, and `control_request_forward` belong to the conversational turn, and a join question is not one. The face feeds the wizard from the join block's `question` field and sends its own CONTROL answer.
...
The one-shot spawn the scribe uses (`claude -p`, prompt on stdin, result out) cannot ask-and-wait, so the resolver spawn is a **multi-turn stream**: `claude -p --input-format stream-json --output-format stream-json --permission-mode acceptEdits` with the read-and-edit tool set explicitly allowed via `--allowedTools` (no Bash — verification is the orchestrator's job, [P05]), cwd = workshop, charter as the first user message. The charter (Spec S01) defines exactly two terminal message shapes, and the orchestrator reads the assistant stream for them:
...
|--------|------|----------|-------|
| `Workshop` | struct | `tugdash-core/src/workshop.rs` | `open_merge` / `open_candidate` / `commit` / `close`, sweep hooks |
...
|---|---|---|---|
| #step-1 | The dash act is named join | pending | — |
| #step-2 | The workshop worktree | pending | — |
| #step-3 | Verification facts and the Tier 0 runner | pending | — |
| #step-4 | Tier 1: the derived selection exam | pending | — |
...
**Artifacts:** renamed `session-changes-dash-landing.tsx` → `session-changes-dash-join.tsx` (plus its `.css`); renamed `at0436-join-land-press.test.ts` → `at0436-join-press.test.ts` and `at0425-dash-conflicted-landing.test.ts` → `at0425-dash-conflicted-join.test.ts`; `@covers` targets repo-wide re-pointed; dash-lane strings and prose swept.
...
- [ ] **Re-point every `@covers` line naming a renamed file.** `session-changes-dash-landing.tsx` is a `@covers` target in at0417, at0418, at0425, at0426, at0436, and at0441; `app-test-covers-check` fails on a `@covers` path that no longer resolves, so the rename is only complete when those six move with it. Grep `@covers` for the old paths, not just the renamed files' own headers.
...
- [ ] Rename the two test files whose own names carry the dash act, headers intact.
...
**Commit:** `join(workshop): a detached scratch worktree where the merge is real`
...
**Artifacts:** `changeset_join_question` / `changeset_join_question_answer` actions; `DashJoinState.question`; face dialog via `TugInlineDialog`.
...
- [ ] Client: changeset store carries the question ([L02]); the join face mounts `TugInlineDialog` with 2–4 options + free text; answer sends the CONTROL action with a typed outcome ([L31]).

**Tests:**
- [ ] Integration (Rust): ask-then-resolve stub round-trips; expiry sticks with reason.
- [ ] tugdeck unit: store carries/clears the question from feed frames; controller submits and reports refusals by type.
...
- [ ] `just app-test at0417-join-mode.test.ts at0418-join-outcomes.test.ts at0425-dash-conflicted-join.test.ts at0436-join-press.test.ts` green
...
- [ ] at0443: a break-the-build stub that never repairs (the iteration budget stays 3 — no test-only knob); assert the loop exhausts, red names the failing command, and **Join anyway** joins past it.
