...
- **Per-file receipt rows for Merge/Rebase joins** — named as follow-on work in `dash/join-receipt-mechanics.md`.
...
- The join receipt work (`dash/join-receipt-mechanics.md`) is landed on `main` — the durable receipt the progress row settles into is the commit-presentation join receipt.
