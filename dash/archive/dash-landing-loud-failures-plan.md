...
- [ ] `dash/continuing-dash-join-failures.md` — no edits; it remains the incident record this plan cites.
...
**Artifacts:**
- `tuglaws/tuglaws.md`: new `## Refusals and Failure Surfaces` section with `### L31. A user gesture produces either the act or a visible reason — never silence. {#l31}` and a grounding paragraph naming the three violation shapes (bare `return` on a gesture, swallowed internal fault, callback parked on an event that may never fire) and citing `dash/continuing-dash-join-failures.md` as the incident that established it.
