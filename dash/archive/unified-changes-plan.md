...
- **Entry points into dash workflows** (starting a dash from a visible surface). That is the next round in `dash/dash-closure-brief.md`.
...
- [ ] `dash/dash-closure-brief.md#unified-changes` — correct the `DashFactsRun` claim per [P05](#p05-bare-names), and record that the doubled-prefix item was half-landed already per [P10](#p10-foreign-scope).
...
3. **Archive the six historical documents** the brief's `#document-map` names, so `dash/` reads as only the live surface.
