...
export interface DashFileTotals {
  files: number;
  added: number;
  deleted: number;
 ...
 */
export function clusterDashFiles(files: readonly ChangesetFile[]): DashFileCluster[] {
  ...
  const byDir = new Map<string, DashFileCluster>();
  for (const file of files) {
...
export function dashFileTotals(files: readonly ChangesetFile[]): DashFileTotals {
  let added = 0;
  let deleted = 0;
  let counted = false;
  for (const file of files) {
