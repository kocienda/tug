 ...
 * the card's {@link DashReplayNoticeController} turns it into a pane bulletin —
 * the same shape {@link dashBindErrorStore} uses, for the same reason ([L22]: a
 * bulletin is a direct DOM update and must not round-trip through render).
 *
 * @module lib/dash-replay-outcome-store
 */

/** The outcome words the server's `changeset_replay_ok` can carry, plus the refusal. */
export type DashReplayOutcomeWord =
  | "current"
  | "replayed"
  | "recorded"
  | "deferred"
...
export interface DashReplayOutcome {
  readonly dash: string;
  readonly outcome: DashReplayOutcomeWord;
...
class DashReplayOutcomeStore {
  private _outcomes = new Map<string, DashReplayOutcome>();
  ...
  outcomeFor = (sessionId: string): DashReplayOutcome | null =>
    this._outcomes.get(sessionId) ?? null;

  /** Record an outcome and wake the readers. */
  report = (
    sessionId: string,
    outcome: Omit<DashReplayOutcome, "seq">,
  ): void => {
    this._seq += 1;
...
export const dashReplayOutcomeStore = new DashReplayOutcomeStore();
