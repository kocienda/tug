/**
 * arc-review-target — where a `/arc-review` invocation points, and what this card
 * last pointed it at.
 *
 * The review is an **ordinary turn on whatever model is selected**. There is no
 * borrow, no scheduling, and no machine: `/arc-review` submits the skill the
 * same way any other slash command does, and `arc-devise` either reviews the
 ...
 * `/arc-review` to a plan, and remembering the last one so the next bare
 * invocation has an answer.
 *
 * @module lib/arc-review-target
 */
...
export const REVIEW_PLAN_COMMAND = "tugplug:arc-review";

/**
 * Remember the plan this card just reviewed, so a later bare `/arc-review`
 * resolves to it.
 *
...
export function writeLastReviewedPlan(cardId: string, planPath: string): void {
  ...
  fetch(url, {
    ...
    console.warn(`[arc-review] PUT failed for card ${cardId}:`, err);
  });
}
