...
describe("SessionLedgerStore", () => {
  ...
  // ---- ensureListed — the wait the deck takes before it measures a picker ----

  describe("ensureListed", () => {
    /** Whether a promise has settled by the next microtask turn. */
    ...
    it("is null for an empty path — nothing to wait for", () => {
      const { store } = newStore();
      expect(store.ensureListed("")).toBeNull();
      store.dispose();
    });

    it("kicks the first fetch for an unseen path and resolves on a phase-1 frame that carries rows", async () => {
      const { store, conn } = newStore();
      const wait = store.ensureListed("ws-1");
      expect(wait).not.toBeNull();
      expect(conn.recordedFrames.filter((f) => f.feedId === FeedId.CONTROL).length).toBe(1);
      expect(await settledYet(wait!)).toBe(false);
      publishListSessionsOk({
        project_dir: "ws-1",
        dir_exists: true,
        scanning: true,
      ...
      expect(await settledYet(wait!)).toBe(true);
      store.dispose();
    });

    it("holds through an EMPTY phase-1 frame and resolves on the settled scan", async () => {
      // A path the ledger knows nothing about is listed by its scan or not at
      // all: a picker measured over the empty phase-1 list would be measured
      // over nothing, and the rows landing later would re-divide the column.
      const { store } = newStore();
      const wait = store.ensureListed("ws-1")!;
      publishListSessionsOk({
        project_dir: "ws-1",
      ...
      publishListSessionsOk({
        ...
        dir_exists: true,
        scanning: false,
        sessions: [makeRow({ session_id: "ext" })],
      });
      expect(await settledYet(wait)).toBe(true);
      store.dispose();
    });

    it("refreshes a settled path and resolves on the refresh's own frame", async () => {
      const { store, conn } = newStore();
      store.getSnapshot("ws-1");
      publishListSessionsOk({
        project_dir: "ws-1",
      ...
      const before = conn.recordedFrames.filter((f) => f.feedId === FeedId.CONTROL).length;
      const wait = store.ensureListed("ws-1")!;
      // The refresh went out, and the snapshot reads as scanning while it does.
      expect(conn.recordedFrames.filter((f) => f.feedId === FeedId.CONTROL).length).toBe(before + 1);
      expect(store.getSnapshot("ws-1").scanning).toBe(true);
      expect(await settledYet(wait)).toBe(false);
      publishListSessionsOk({
        project_dir: "ws-1",
        dir_exists: true,
    ...
    it("joins a request already in flight rather than sending another", async () => {
      const { store, conn } = newStore();
      store.getSnapshot("ws-1");
      const before = conn.recordedFrames.filter((f) => f.feedId === FeedId.CONTROL).length;
      const wait = store.ensureListed("ws-1")!;
      expect(conn.recordedFrames.filter((f) => f.feedId === FeedId.CONTROL).length).toBe(before);
      publishListSessionsOk({
        project_dir: "ws-1",
        dir_exists: true,
        sessions: [makeRow({ session_id: "tug" })],
    ...
    it("resolves on an error frame, and is null for a path already in error", async () => {
      const { store } = newStore();
      const wait = store.ensureListed("ws-1")!;
      publishListSessionsErr({ project_dir: "ws-1", reason: "nope" });
      expect(await settledYet(wait)).toBe(true);
      expect(store.ensureListed("ws-1")).toBeNull();
      store.dispose();
    });
  });
