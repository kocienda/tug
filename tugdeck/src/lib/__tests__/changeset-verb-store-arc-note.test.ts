 ...
 * a `tugtool arc` verb is a short-lived process with nobody waiting. What is
...
describe("dash_note", () => {
  test("a gesture is filed under the session it names, with its row identity", () => {
    ...
    expect(h.store.arcNotes("somebody-else").length).toBe(0);
  });
  ...
  test("a frame with nothing to say is not a row", () => {
    ...
    expect(h.store.arcNotes(SESSION).length).toBe(0);
  });
  ...
  test("an older tugcast that persisted nothing still paints", () => {
    ...
    expect(h.store.arcNotes(SESSION)[0].receiptId).toBeNull();
  });
