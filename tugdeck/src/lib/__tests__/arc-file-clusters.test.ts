...
describe("clusterArcFiles", () => {
  ...
  test("two files at the root cluster at the root; one alone keeps the root too", () => {
    expect(clusterArcFiles([file("a.md"), file("b.md")]).map((c) => c.dir)).toEqual([""]);
    expect(clusterArcFiles([file("a.md")]).map((c) => c.dir)).toEqual([""]);
  });
  ...
  test("the grouping does not depend on arrival order", () => {
    const a = clusterArcFiles([file("p/q/one.ts"), file("p/two.ts"), file("p/q/three.ts")]);
    const b = clusterArcFiles([file("p/two.ts"), file("p/q/three.ts"), file("p/q/one.ts")]);
    expect(a.map((c) => [c.dir, c.files.length])).toEqual(b.map((c) => [c.dir, c.files.length]));
    expect(a.map((c) => c.dir)).toEqual(["p/q", "p"]);
  });
});

describe("arcFileTotals", () => {
  ...
  test("an uncounted list is not a list of zero-line changes", () => {
    expect(arcFileTotals([file("a"), file("b")])).toEqual({
      files: 2,
      added: 0,
      deleted: 0,
      counted: false,
