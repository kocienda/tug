 ...
 * 2. Failing that, the deck's first responder — the single-card behavior that
 *    predates the selection, now the degenerate case rather than the rule.
 * 3. Failing that, nothing.
 *
 * Rung 1 is what makes the chords work with keyboard focus in the Lens. The
 * first responder there is the Lens card itself — a rail — so a handler reading
 * only rung 2 refuses every slot chord typed while the Cards list has focus,
 * which is exactly when the user is most likely to type one.
 *
