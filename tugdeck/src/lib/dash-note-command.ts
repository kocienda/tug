...
export const DASH_NOTE_COMMAND = /^dash (create|step|mark|commit)\s+\S+/;

/** Whether a shell-ledger command is a dash gesture's synthetic quiet line. */
export function matchesDashNote(command: string): boolean {
 ...
 */
export function dashNoteSentence(message: { command: string; output: string }): string {
