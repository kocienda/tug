 ...
 *
 * The binding commit used to be where an unbound Session card's declared
 * height came down, and it is not any more: a card's opening bid is ended by
 * the sheet it was declared for — superseded by a measurement at least as
 * high, cleared when the sheet goes — so no card-state transition is party to
 * it. Binding is what makes a picker go, and the sheet going is what
 * `sheetClaimWith` reads. See `lib/opening-bid.ts`.
 *
