...
/* ---------------------------------------------------------------------------
 * PlaceFitGlyph / PlaceFlowGlyph
 * ---------------------------------------------------------------------------*/

/**
 * The run divided and ENDING where the run ends — fit, drawn as the thing it
 * is: bands that between them come to exactly the field's height, so the last
 * one is flush with the foot ([B09]).
 *
 * It is {@link SplitGlyph}'s drawing in a second vocabulary on purpose. Split
 * and stack answer "is this place divided"; fit and flow answer "does the
 * division fill the run or run past it", and the second question is only asked
 * of a place the first one already divided. Drawing fit as the split it already
 * is, and flow as that same split overrunning the box, is what makes the pair
 * read as one more fact about the same picture rather than as a new one.
 */
export function PlaceFitGlyph(): React.ReactElement {
  return (
    <svg
      className="tug-column-badge-glyph"
      viewBox="0 0 16 20"
...
/**
 * The same strip, pinned at the top and RUNNING OFF the foot — flow, whose
 * members stand at the heights their own content asked for and whose strip is
 * therefore as long as that makes it ([B08]).
 *
 * The last band is drawn past the viewBox and clipped by it, which is the
 * whole of the difference from {@link PlaceFitGlyph} and is the same thing the
 * real rail does: the run is a window, and what does not fit is below the fold
 * rather than squeezed into it. It is drawn at a tighter pitch than fit's so
 * the clipped band is unmistakably a fourth one rather than fit's third one
 * mis-drawn.
 */
export function PlaceFlowGlyph(): React.ReactElement {
  return (
    <svg
      className="tug-column-badge-glyph"
      viewBox="0 0 16 20"
