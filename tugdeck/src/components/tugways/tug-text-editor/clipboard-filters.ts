...
export function serializeClipboard(
  ...
  if (local.length > 0) {
    ...
    // its `atomPlainText` spelling, the same table the substrate copy and the
    // annotation menu read, so the editor is not the one door that answers
    // this question differently — walking back-to-front so earlier
