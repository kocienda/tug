...
import {
  ...
  type DashReplayOutcome,
} from "@/lib/dash-replay-outcome-store";
...
function conflictDescription(outcome: DashReplayOutcome): string {
  const round =
...
export function DashReplayNoticeController({
  tugSessionId,
}: {
  ...
  useLayoutEffect(() => {
    const apply = (): void => {
      const outcome = dashReplayOutcomeStore.outcomeFor(tugSessionId);
      if (outcome === null) {
      ...
      switch (outcome.outcome) {
        case "current":
          ...
          return;
        case "deferred":
          api.caution("Couldn't replay that dash", {
            id: NOTICE_ID,
    ...
    apply();
    return dashReplayOutcomeStore.subscribe(apply);
  }, [api, tugSessionId]);
