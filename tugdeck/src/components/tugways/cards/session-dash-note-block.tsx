/**
 * `SessionDashNoteBlock` — a dash run's quiet lines on the `$`-route, the
 ...
 * synthetic tell: a command a user actually typed spells `tugtool dash …`.
 *
...
import { dashNoteSentence, matchesDashNote } from "@/lib/dash-note-command";
import {
  registerCommandBlock,
  type CommandBlockProps,
} from "./session-command-block-registry";
import { formatTranscriptTimestamp } from "./transcript-host-helpers";
import "./session-dash-note-block.css";

export { matchesDashNote };

export function SessionDashNoteBlock({ message }: CommandBlockProps): React.ReactElement {
  ...
  return (
    ...
    >
      <TugQuietLine
        ...
        subject={<span data-tugx-findable="">{dashNoteSentence(message)}</span>}
        tone="quiet"
      />
    </div>
  );
 ...
 */
export function dashNoteFindParts(message: ShellExchangeMessage): string[] {
  return [dashNoteSentence(message)];
}
...
registerCommandBlock("dash-note", matchesDashNote, SessionDashNoteBlock, {
  attribution: "wheel",
  presentation: "quiet",
  findParts: dashNoteFindParts,
});
