...
describe("orderArcLane", () => {
  ...
  test("the bound owner key fronts its arc", () => {
    const bound = ARCS[0]!;
    const order = orderArcLane(ARCS, bound.owner_id);
  ...
  test("an unbound card fronts nothing and folds everything", () => {
    const order = orderArcLane(ARCS, null);
  ...
  test("a binding to a dead incarnation fronts nothing", () => {
    ...
    const order = orderArcLane(ARCS, stale);
    expect(order.fronted).toBeNull();
    expect(order.rest).toEqual(ARCS);
  });
  ...
  test("fronting follows the id it is given, which need not be the binding", () => {
    ...
    const aimed = ARCS[0]!;
    const order = orderArcLane(ARCS, aimed.owner_id);
  ...
  test("an empty lane orders to nothing", () => {
    const order = orderArcLane([], "tugarc/whatever#1");
    expect(order.fronted).toBeNull();
  ...
  test("later arcs front just as well as the first", () => {
    ...
    const order = orderArcLane([ARCS[0]!, second], second.owner_id);
