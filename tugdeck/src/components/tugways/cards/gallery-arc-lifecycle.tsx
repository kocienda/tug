 ...
 *
 *   - `TugArcAtom` — the one skin, two sizes. A direct dash is a dash to it.
 ...
 * whose subject IS the dash (the rail's Arcs section, the Changes shade, the
...
const SOLO_PLAN = `${ROOT}/.tug/arcs/${SOLO_DASH}/plan.md`;
...
const MOMENTS: readonly Moment[] = [
  ...
  },
  {
    key: "direct-listed",
    caption:
      "A DIRECT dash — no wheel driving it — the moment its task list is written and before any round lands. TWO cells, because devise and review were never skipped: a dash never has them, and a five-cell strip with two struck out would say otherwise. The fraction is real from the first frame: the task list is an ordinary plan document, so `0/3` is counted rather than stood in for",
    workers: [SOLO],
  ...
  },
  {
    key: "direct-working",
    caption:
      "The same dash mid-walk, step 2 of 3. Its implement cell holds the same three-cell width a trek's does and its ticks divide it, so the two read as one instrument at two plan lengths — nothing about the strip says which session is driving. Who is on it is the atom beside it",
    workers: [SOLO],
    prompt: `/dash ${SOLO_DASH} ${SOLO_SUBJECT}`,
    branched: true,
    entry: entry(SOLO_DASH, {
  ...
  },
  {
    key: "direct-ready",
    caption:
      "Every task closed and the draft written — the join, offered on the same terms a trek's is. The join cell fills and the phase glyph turns over; nothing here distinguishes the two kinds, because at the join there is nothing left to distinguish",
    workers: [SOLO],
    prompt: `/dash ${SOLO_DASH} ${SOLO_SUBJECT}`,
    branched: true,
    entry: entry(SOLO_DASH, {
  ...
  },
  {
    key: "direct-listless",
    caption:
      "An arc with no documents at all — every dash cut before task lists existed, and any run that skipped writing one. The strip is the same three cells and the implement cell is bare, because there is nothing to divide it into. This is the one case that still reads a WORD where the others read numbers",
    workers: [SOLO],
    prompt: `/dash ${SOLO_DASH} ${SOLO_SUBJECT}`,
    branched: true,
    entry: entry(SOLO_DASH, {
...
function briefEntry(message: string, source: string): DashChangesetEntry {
  return entry(BRIEF_DASH, {
    ...
    step_total: 4,
    documents: { plan: `${ROOT}/.tug/arcs/${BRIEF_DASH}/plan.md` },
    task_list: true,
...
function blockedEntry(blocker: ArcJoinBlockerWire): DashChangesetEntry {
  ...
  return entry(BLOCKED_DASH, {
    ...
    step_total: 6,
    documents: { plan: `${ROOT}/.tug/arcs/${BLOCKED_DASH}/plan.md` },
    task_list: true,
    rounds: 6,
    files: [
      {
  ...
  return (
    <div className="cg-content" data-testid="gallery-dash-lifecycle">
      <section className="cg-section">
        ...
        <Stage caption="One skin, two registers (prose in a line of running text · reading in a block), proportional everywhere; who is on it is the atom beside it. A direct dash is a dash to the atom: both are work on a worktree">
          <div className="cg-dash-lineup">
            <TugArcAtom name={DASH} register="prose" />
            <TugArcAtom name={DASH} register="reading" />
      ...
      <section className="cg-section">
        ...
        <Stage caption="The same five phases as one glyph — ArcPhaseMark. Keyed on the lifecycle PHASE, never on the git stage: a dash devising or reviewing a plan has no stage at all, which is how the mark that used to do this job came to be blank for the whole first half of a dash's life">
          <div className="cg-dash-legend-row">
            {MOMENTS.map((m) => (
              <ArcPhaseMark
                key={m.key}
