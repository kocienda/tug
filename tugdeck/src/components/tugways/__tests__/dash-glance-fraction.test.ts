...
describe("the pair the numerals count", () => {
  test("a declared run wins over the plan's own counters", () => {
    ...
    expect(dashGlanceFraction(1, 3, 1, 10)).toEqual({ current: 1, total: 3 });
  });

  test("a mid-plan run counts from its own start, not the plan's", () => {
    // Steps 5-7 with step 6 open is the second of three, never `6/7`.
    expect(dashGlanceFraction(2, 3, 6, 10)).toEqual({ current: 2, total: 3 });
  });

  test("no declared run falls back to the plan pair, exactly as before", () => {
    expect(dashGlanceFraction(null, null, 4, 9)).toEqual({
      current: 4,
      total: 9,
    });
    expect(dashGlanceFraction(undefined, undefined, 4, 9)).toEqual({
      current: 4,
      total: 9,
    });
  });
  ...
  test("a half-sent run pair is not a pair, and falls back", () => {
    expect(dashGlanceFraction(2, null, 6, 10)).toEqual({
      current: 6,
      total: 10,
    });
    expect(dashGlanceFraction(null, 3, 6, 10)).toEqual({
      current: 6,
      total: 10,
    });
  });
  ...
  test("neither pair declared renders no counter at all", () => {
    expect(dashGlanceFraction(null, null, null, null)).toBeNull();
    expect(
      dashGlanceFraction(undefined, undefined, undefined, undefined),
    ).toBeNull();
  });
});
...
describe("the run's span across the plan", () => {
  test("a run at the plan's head starts there", () => {
    ...
    expect(dashRunScope(1, 3, 1, 10)).toEqual({ from: 1, through: 3 });
  });
  ...
  test("a mid-plan run sits where it belongs in the document", () => {
    ...
    expect(dashRunScope(2, 3, 6, 10)).toEqual({ from: 5, through: 7 });
  });
  ...
  test("a run spanning the whole plan covers every segment", () => {
    ...
    expect(dashRunScope(1, 1, 1, 1)).toEqual({ from: 1, through: 1 });
    expect(dashRunScope(3, 10, 3, 10)).toEqual({ from: 1, through: 10 });
  });

  test("no declared run means no band", () => {
    expect(dashRunScope(null, null, 4, 9)).toBeUndefined();
    expect(dashRunScope(undefined, undefined, undefined, undefined)).toBeUndefined();
  });
  ...
  test("a span that would fall outside the plan is refused, not clamped", () => {
    ...
    expect(dashRunScope(5, 3, 2, 10)).toBeUndefined();
    expect(dashRunScope(1, 12, 1, 10)).toBeUndefined();
  });
});
