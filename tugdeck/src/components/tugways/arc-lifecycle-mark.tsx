/**
 * ArcLifecycleMark — an arc's life at a glance: pill · glyph · fraction.
 *
 * The compact register of the lifecycle grammar, for the two surfaces where a
 * arc is not the subject — the session masthead's title line and the Arcs card's
 ...
 * pair, `data-slot`; [L20] composes `ArcPhaseMark`, `TugStepFraction`, and
 * `TugTooltip`.
 *
 * @module components/tugways/arc-lifecycle-mark
 */

import "./arc-lifecycle-mark.css";
...
import { arcCellState, type ArcTrackModel } from "./tug-arc-track";
 ...
 */
export function arcMarkFraction(
  model: ArcTrackModel,
...
export interface ArcLifecycleMarkProps {
  model: ArcTrackModel;
...
export function ArcLifecycleMark({
  model,
  size = "rail",
  name,
  fraction: fractionOverride,
}: ArcLifecycleMarkProps): React.ReactElement {
  const state = arcCellState(model, model.phase);
  const fraction =
