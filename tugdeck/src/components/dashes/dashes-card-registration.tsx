/**
 * dashes-card-registration.tsx — registers the Dashes card ([L25]).
 *
 ...
 *
 * INVARIANT: `registerDashesCard()` MUST run at boot unconditionally and before
 ...
 *
 * `family: "dashes"` (a family no free pane's `acceptsFamilies` lists) plus
 * `acceptsFamilies: []` makes the card un-mergeable in both directions.
 *
 * @module components/dashes/dashes-card-registration
 */

import React from "react";
import { registerCard } from "@/card-registry";
import { DASHES_CARD_ID } from "@/lib/dashes-card-id";
import { DashesContent } from "./dashes-card";
...
/** The narrowest a dash's two-line lifecycle block still reads at. */
export const MIN_DASHES_WIDTH_PX = 320;

/** Register the Dashes card. `hidden` keeps it out of the type-picker `[+]`
 *  menu — it is reachable through its own menu row and the rail ladder. */
export function registerDashesCard(): void {
  registerCard({
    componentId: DASHES_CARD_ID,
    family: "dashes",
    acceptsFamilies: [],
    contentFactory: (cardId: string) => <DashesContent cardId={cardId} />,
    // A dash IS a branch plus a worktree, and this is the glyph that says so.
    defaultMeta: { title: "Dashes", icon: "GitBranch", closable: true },
    greedRank: 2,
    hidden: true,
    ...
    cardsGroup: "none",
    sizePolicy: {
      min: { width: MIN_DASHES_WIDTH_PX, height: 240 },
      preferred: { width: DEFAULT_DASHES_WIDTH_PX, height: 900 },
    },
  });
}
