/**
 * dashes-card.tsx — the **Dashes** card: every dash in every open project, in
 * every state, always on.
 *
 * The card is a fixed address. It used to show only unbound dashes and to
 ...
 * dash is a row, and the empty state is one quiet line that costs ~24px and
 ...
 * projection of it and nothing more. Rows key on the dash's **owner key**, which makes two
 ...
 * mates an unbound dash to the card's followed card, or carries its refusal in
 ...
 *
 * Beneath the dashes the card lists the **waiting paperwork**: plan
 ...
 * the line.
 *
 * @module components/dashes/dashes-card
 */

import "./dashes-card.css";
...
export interface DashesContentProps {
  /** The Dashes card's id. */
  cardId: string;
}

export function DashesContent({ cardId }: DashesContentProps): React.ReactElement {
