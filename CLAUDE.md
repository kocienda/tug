...
`session_index.db` is the machine-global index of **where sessions live** — one row per session segment, carrying only the facts needed to *find* it (uuid, line, callsign, project dir, recording instance, title). It exists because `sessions.db` is per-instance and a callsign is unique only within one of those, so a reference to a session another instance recorded, or one in a project this instance has never opened, is otherwise unanswerable; the index is what lets such a reference resolve to "elsewhere on this machine" rather than to nothing. Instance isolation of `sessions.db` is untouched — the index holds the finding facts and never the session's truth. It lives in `tugcore` (`tugrust/crates/tugcore/src/session_index.rs`), not tugcast, because tugtool reads it and cannot link tugcast. **tugcast is the only writer**, at spawn, rename, auto-title and trash, and its writes are telemetry-grade: a failed index write is logged and never fails the ledger operation it rides. Every other reader opens read-only, where a missing file or table is an empty answer rather than an error. Inspect it like the others (`just db-inspect session_index "SELECT …"`); its schema is gated on `PRAGMA user_version` with a registered migration list, the same regime as `prompt_history.db`. `TUG_SESSION_INDEX_DB` redirects it for test isolation — set it in any harness that spawns tugcast or tugtool, or the run writes rows the user's real instances will resolve as real sessions.
