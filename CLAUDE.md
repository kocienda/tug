...
- The `dash-implement` and `cut` skills commit on their **dash worktree** (never on `main`) via `tugutil dash commit`, as part of running a recipe / dash. `main` is only updated by the user's landing gestures.
...
| `tugplug/` | Claude Code plugin (agentless skills: dash/dash-devise/dash-review/dash-implement/cut/draft/spike-card). A dash's brief and plan live at `.tug/dashes/<name>/` and are never tracked. |
